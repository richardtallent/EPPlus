...
internal struct IndexItem : IComparable<IndexItem> {
	...
	internal short Index;
	public int CompareTo(IndexItem other) => Index - other.Index;
}
		...
		if (p < 0) {
			...
			if (p >= PageCount) {
				...
				if (_pages[p].IndexOffset + _pages[p].Rows[0].Index < row) {
					return p + 1 >= PageCount ? -1 : _pages[p + 1].IndexOffset + _pages[p].Rows[0].Index;
				} else {
			...
			if (p < PageCount) {
				...
				if (r >= 0) {
					...
					return ++p < PageCount ? _pages[p].IndexOffset + _pages[p].Rows[0].Index : -1;
				}
			} else {
				return -1;
			}
...
internal class CellStore<T> : IDisposable// : IEnumerable<ulong>, IEnumerator<ulong>
{
	...
	public CellStore() => _columnIndex = new ColumnIndex[ColSizeMin];
	~CellStore() {
		if (_values != null) {
			_values.Clear();
			_values = null;
	...
	internal CellStore<T> Clone() {
		...
		for (var c = 0; c < ColumnCount; c++) {
			...
			for (var p = 0; p < _columnIndex[c].PageCount; p++) {
				for (var r = 0; r < _columnIndex[c]._pages[p].RowCount; r++) {
			...
			for (var c = fromIndex; c < ColumnCount; c++) {
				...
				if (_columnIndex[c]._pages[0].RowCount > 0 && _columnIndex[c]._pages[0].Rows[0].Index > 0) {
					...
					if (_columnIndex[c]._pages[0].RowCount > 1) {
						...
						first = _columnIndex[c].PageCount > 1 ? _columnIndex[c]._pages[0].IndexOffset + _columnIndex[c]._pages[1].Rows[0].Index : 0;
					}
				}
				...
				last = p.RowCount > 0 ? p.IndexOffset + p.Rows[p.RowCount - 1].Index : first;
	...
	int GetPointer(int Row, int Column) {
		...
		if (col >= 0) {
			...
			if (pos >= 0 && pos < _columnIndex[col].PageCount) {
				...
				return cellPos >= 0 ? pageItem.Rows[cellPos].IndexPointer : -1;
			} else //Page does not exist
			  {
				return -1;
			}
	...
	internal bool Exists(int Row, int Column) => GetPointer(Row, Column) >= 0;
			...
			if (columns > 0) {
				...
				for (var c = 0; c < ColumnCount; c++) {
					...
					if (pagePos >= 0) {
						...
						{
							var offset = fromRow - column._pages[pagePos].IndexOffset;
	...
	private static bool IsWithinPage(int row, ColumnIndex column, int pagePos) => (row >= column._pages[pagePos].MinIndex && row <= column._pages[pagePos].MaxIndex);

	internal void Clear(int fromRow, int fromCol, int rows, int columns) => Delete(fromRow, fromCol, rows, columns, false);
	internal void Delete(int fromRow, int fromCol, int rows, int columns) => Delete(fromRow, fromCol, rows, columns, true);
	...
	private void UpdateIndexOffset(ColumnIndex column, int pagePos, int rowPos, int row, int rows) {
		...
		if (rows > PageSize) {
			...
			var offset = +(int)(rows - (PageSize * addPages));
	...
	int _colPos = -1, _row;
	public ulong Current => ((ulong)_row << 32) | (ulong)(long)(_columnIndex[_colPos].Index);
	...
	//    }
	//}
	public bool MoveNext() => GetNextCell(ref _row, ref _colPos, 0, ExcelPackage.MaxRows, ExcelPackage.MaxColumns);
	internal bool NextCell(ref int row, ref int col) => NextCell(ref row, ref col, 0, 0, ExcelPackage.MaxRows, ExcelPackage.MaxColumns);
	...
	internal bool PrevCell(ref int row, ref int col) => PrevCell(ref row, ref col, 0, 0, ExcelPackage.MaxRows, ExcelPackage.MaxColumns);
...
internal class FlagCellStore : CellStore<byte> {
	...
	internal bool GetFlagValue(int Row, int Col, CellFlags cellFlags) => !(((byte)cellFlags & GetValue(Row, Col)) == 0);
}
