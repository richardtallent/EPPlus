	...
	public abstract class ExcelConditionalFormattingRule
	  ...
	  IExcelConditionalFormattingRule {
			...
			set {
				...
				if (priority != value) {
					...
					if (!_changingPriority) {
						...
						if (priority > value) {
							for (var i = priority - 1; i >= value; i--) {
		...
		public UInt16 StdDev {
			get => Convert.ToUInt16(GetXmlNodeInt(
		...
		internal protected bool? AboveAverage {
			get {
				var aboveAverage = GetXmlNodeBoolNullable(
			...
			set {
				var aboveAverageValue = string.Empty;
		...
		internal protected bool? EqualAverage {
			get {
				var equalAverage = GetXmlNodeBoolNullable(
			...
			set {
				var equalAverageValue = string.Empty;
		...
		internal protected bool? Bottom {
			get {
				var bottom = GetXmlNodeBoolNullable(
		...
		internal protected bool? Percent {
			get {
				var percent = GetXmlNodeBoolNullable(
		...
		internal protected eExcelConditionalFormattingTimePeriodType TimePeriod {
			get => ExcelConditionalFormattingTimePeriodType.GetTypeByAttribute(
		...
		internal protected eExcelConditionalFormattingOperatorType Operator {
			get => ExcelConditionalFormattingOperatorType.GetTypeByAttribute(
