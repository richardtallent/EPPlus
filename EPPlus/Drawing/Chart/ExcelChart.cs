	...
	public class ExcelChart : ExcelDrawing {
		...
		private void SetTypeProperties() {
			...
			if (IsType3D()) {
				...
				View3D.RotX = IsTypePieDoughnut() ? 30 : 15;
			}
		}
		private void CreateNewChart(ExcelDrawings drawings, eChartType type, ExcelChart topChart) {
			if (topChart == null) {
				...
				var streamChart = new StreamWriter(Part.GetStream(FileMode.Create, FileAccess.Write));
				...
				if (axNode != null && axNode.Count > 1) {
					foreach (XmlNode axn in axNode) {
						if (axn.ParentNode.LocalName.EndsWith("Ax")) {
							...
							var ax = new ExcelChartAxis(NameSpaceManager, axisNode);
							l.Add(ax);
						}
					}
				}
		...
		private string AddScatterType(eChartType type) => type == eChartType.XYScatter ||
				...
				? "<c:scatterStyle val=\"\" />"
				: "";
		private string AddRadarType(eChartType type) => type == eChartType.Radar ||
		...
		private string AddHoleSize(eChartType type) => type == eChartType.Doughnut ||
				type == eChartType.DoughnutExploded
				? "<c:holeSize val=\"50\" />"
				: "";
		private string AddFirstSliceAng(eChartType type) => type == eChartType.Doughnut ||
				type == eChartType.DoughnutExploded
				? "<c:firstSliceAng val=\"0\" />"
				: "";
		private string AddVaryColors() => IsTypePieDoughnut() ? "<c:varyColors val=\"1\" />" : "<c:varyColors val=\"0\" />";
		private string AddHasMarker(eChartType type) => type == eChartType.LineMarkers ||
		...
		internal static bool IsType3D(eChartType chartType) => chartType == eChartType.Area3D ||
		...
		protected bool IsTypeLine() => ChartType == eChartType.Line ||
					...
					ChartType == eChartType.Line3D;
		protected bool IsTypeScatterBubble() => ChartType == eChartType.XYScatter ||
		...
		protected bool IsTypeSurface() => ChartType == eChartType.Surface ||
				   ChartType == eChartType.SurfaceTopView ||
				   ChartType == eChartType.SurfaceTopViewWireframe ||
				   ChartType == eChartType.SurfaceWireframe;
		protected bool IsTypeShape() => ChartType == eChartType.BarClustered3D ||
					ChartType == eChartType.BarStacked3D ||
					ChartType == eChartType.BarStacked1003D ||
					ChartType == eChartType.BarClustered3D ||
					ChartType == eChartType.BarStacked3D ||
					...
					ChartType == eChartType.PyramidColClustered ||
					ChartType == eChartType.PyramidColStacked ||
					ChartType == eChartType.PyramidColStacked100; //||//ChartType == eChartType.Doughnut ||//ChartType == eChartType.DoughnutExploded;
		protected internal bool IsTypePercentStacked() => ChartType == eChartType.AreaStacked100 ||
						   ...
						   ChartType == eChartType.PyramidBarStacked100 ||
						   ChartType == eChartType.PyramidColStacked100;
		protected internal bool IsTypeStacked() => ChartType == eChartType.AreaStacked ||
						   ChartType == eChartType.AreaStacked3D ||
						   ChartType == eChartType.BarStacked ||
						   ChartType == eChartType.BarStacked3D ||
						   ChartType == eChartType.ColumnStacked3D ||
						   ...
						   ChartType == eChartType.PyramidBarStacked ||
						   ChartType == eChartType.PyramidColStacked;
		protected bool IsTypeClustered() => ChartType == eChartType.BarClustered ||
						   ChartType == eChartType.BarClustered3D ||
						   ChartType == eChartType.ColumnClustered3D ||
						   ...
						   ChartType == eChartType.PyramidBarClustered ||
						   ChartType == eChartType.PyramidColClustered;
		protected internal bool IsTypePieDoughnut() => ChartType == eChartType.Pie ||
		...
		public eChartStyle Style {
			...
			set {
				if (value == eChartStyle.None) {
					if (ChartXml.SelectSingleNode("c:chartSpace/c:style", NameSpaceManager) is XmlElement element) {
		...
		public bool RoundedCorners {
			get => _chartXmlHelper.GetXmlNodeBool(_roundedCornersPath);
			set => _chartXmlHelper.SetXmlNodeBool(_roundedCornersPath, value);
		}
		...
		public bool ShowHiddenData {
			get =>
				//!!Inverted value!!
				!_chartXmlHelper.GetXmlNodeBool(_plotVisibleOnlyPath);
			set =>
				//!!Inverted value!!
		...
		public eDisplayBlanksAs DisplayBlanksAs {
			...
			set => _chartSeries.SetXmlNodeString(_displayBlanksAsPath, value.ToString().ToLower(CultureInfo.InvariantCulture));
		}
		...
		public bool ShowDataLabelsOverMaximum {
			get => _chartXmlHelper.GetXmlNodeBool(_showDLblsOverMax, true);
			set => _chartXmlHelper.SetXmlNodeBool(_showDLblsOverMax, value, true);
		}
		...
		public eGrouping Grouping {
			get => GetGroupingEnum(_chartXmlHelper.GetXmlNodeString(_groupingPath));
			internal set => _chartXmlHelper.SetXmlNodeString(_groupingPath, GetGroupingText(value));
		}
		...
		public bool VaryColors {
			get => _chartXmlHelper.GetXmlNodeBool(_varyColorsPath);
			set {
				if (value) {
			...
			switch (chartNode.LocalName) {
				...
				case "doughnutChart":
					return topChart == null
						...
						: new ExcelDoughnutChart(topChart, chartNode);
				case "pie3DChart":
				case "pieChart":
					return topChart == null
						? new ExcelPieChart(drawings, node, uriChart, part, chartXml, chartNode)
						: new ExcelPieChart(topChart, chartNode);
				case "ofPieChart":
					return topChart == null
				...
				case "line3DChart":
					return topChart == null
						...
						: new ExcelLineChart(topChart, chartNode);
				case "scatterChart":
					return topChart == null
						? new ExcelScatterChart(drawings, node, uriChart, part, chartXml, chartNode)
						: new ExcelScatterChart(topChart, chartNode);
				default:
					return null;
			}
		}
