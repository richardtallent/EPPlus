...
namespace OfficeOpenXml.Drawing.Chart {
	...
	public sealed class ExcelChartAxis : XmlHelper {
		...
		internal ExcelChartAxis(XmlNamespaceManager nameSpaceManager, XmlNode topNode) :
			base(nameSpaceManager, topNode) => SchemaNodeOrder = new string[] { "axId", "scaling", "logBase", "orientation", "max", "min", "delete", "axPos", "majorGridlines", "minorGridlines", "title", "numFmt", "majorTickMark", "minorTickMark", "tickLblPos", "spPr", "txPr", "crossAx", "crossesAt", "crosses", "crossBetween", "auto", "lblOffset", "majorUnit", "majorTimeUnit", "minorUnit", "minorTimeUnit", "dispUnits", "spPr", "txPr" };
		...
		public eAxisPosition AxisPosition {
			...
			internal set => SetXmlNodeString(AXIS_POSITION_PATH, value.ToString().ToLower(CultureInfo.InvariantCulture).Substring(0, 1));
		}
		...
		public string Format {
			...
			set {
				...
				SourceLinked = string.IsNullOrEmpty(value);
			}
		}
		...
		public double? MajorUnit {
			get {
				return AxisType == eAxisType.Cat ? GetXmlNodeDoubleNull(_majorUnitCatPath) : GetXmlNodeDoubleNull(_majorUnitPath);
			}
			set {
				if (value == null) {
		...
		public double? MinorUnit {
			get {
				return AxisType == eAxisType.Cat ? GetXmlNodeDoubleNull(_minorUnitCatPath) : GetXmlNodeDoubleNull(_minorUnitPath);
			}
			set {
				if (value == null) {
		...
		        /// </summary> 
		public void RemoveGridlines() => RemoveGridlines(true, true);
