...
namespace OfficeOpenXml.Drawing.Chart {
	...
	public class ExcelLineChart : ExcelChart {
		...
		internal ExcelLineChart(ExcelDrawings drawings, XmlNode node, eChartType type, ExcelChart topChart, ExcelPivotTable PivotTableSource) :
			base(drawings, node, type, topChart, PivotTableSource) => Smooth = false;
		#endregion
			...
			if (name == "lineChart") {
				if (Marker) {
					if (Grouping == eGrouping.Stacked) {
						...
						return Grouping == eGrouping.PercentStacked ? eChartType.LineMarkersStacked100 : eChartType.LineMarkers;
					}
				} else {
					if (Grouping == eGrouping.Stacked) {
						return eChartType.LineStacked;
					} else {
						return Grouping == eGrouping.PercentStacked ? eChartType.LineStacked100 : eChartType.Line;
					}
				}
