	...
	public sealed class ExcelLineChartSerie : ExcelChartSerie {
		...
		public Color LineColor {
			get {
				...
				return color == "" ? Color.Black : Color.FromArgb(Convert.ToInt32(color, 16));
			}
			set => SetXmlNodeString(LINECOLOR_PATH, value.ToArgb().ToString("X").Substring(2), true);
		}
		...
		public Color MarkerLineColor {
			...
			set => SetXmlNodeString(MARKERLINECOLOR_PATH, value.ToArgb().ToString("X").Substring(2), true);
		}
