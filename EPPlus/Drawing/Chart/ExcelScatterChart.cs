			...
			if (name == "scatterChart") {
				if (ScatterStyle == eScatterStyle.LineMarker) {
					if (((ExcelScatterChartSerie)Series[0]).Marker == eMarkerStyle.None) {
						...
						return ExistNode("c:ser/c:spPr/a:ln/noFill") ? eChartType.XYScatter : eChartType.XYScatterLines;
					}
				} else if (ScatterStyle == eScatterStyle.SmoothMarker) {
					return ((ExcelScatterChartSerie)Series[0]).Marker == eMarkerStyle.None
						? eChartType.XYScatterSmoothNoMarkers
						: eChartType.XYScatterSmooth;
				}
			}
