  ...
  throw (new InvalidOperationException("Can't set marker style for this charttype."));
		...
		public int MarkerSize {
			get {
				...
				return size == "" ? 5 : Int32.Parse(GetXmlNodeString(MARKERSIZE_PATH));
			}
			set {
				var size = value;
		...
		public double LineWidth {
			get {
				...
				return size == "" ? 2.25 : double.Parse(GetXmlNodeString(LINEWIDTH_PATH)) / 12700;
			}
		...
		private void setAlphaChannel(Color c, string xPath) {
			...
			if (c.A != 255) { //opaque color => alpha == 255 //source: https://msdn.microsoft.com/en-us/library/1hstcth9%28v=vs.110%29.aspx
							  //check path
				var s = xPath4Alpha(xPath);
				if (s.Length > 0) {
					var alpha = ((c.A == 0) ? 0 : (100 - c.A) * 1000).ToString(); //note: excel writes 100% transparency (alpha=0) as "0" and not as "100000"
		...
		private string xPath4Alpha(string xPath) {
			...
			if (xPath.EndsWith("/")) { //cut tailing slash
