...
namespace OfficeOpenXml.Drawing {
	...
	public class ExcelDrawing : XmlHelper, IDisposable {
		...
		internal ExcelDrawing(ExcelDrawings drawings, XmlNode node, string nameXPath) :
			...
			To = node != null ? new ExcelPosition(drawings.NameSpaceManager, posNode, GetPositionSize) : null;
			GetPositionSize();
		...
		public string Name {
			get {
				try {
					return _nameXPath == "" ? "" : GetXmlNodeString(_nameXPath);
				} catch {
					return "";
				}
			}
		...
		internal static ExcelDrawing GetDrawing(ExcelDrawings drawings, XmlNode node) {
			...
			//}
				...
				return node.SelectSingleNode("xdr:graphicFrame", drawings.NameSpaceManager) != null
					? ExcelChart.GetChart(drawings, node)
					: new ExcelDrawing(drawings, node, "");
			}
		}
		...
		internal int GetPixelLeft() {
			...
			var pix = 0;
			for (var col = 0; col < From.Column; col++) {
		...
		internal int GetPixelWidth() {
			...
			var pix = -From.ColumnOff / EMU_PER_PIXEL;
			for (var col = From.Column + 1; col <= To.Column; col++) {
		...
		internal int GetPixelHeight() {
			...
			var pix = -(From.RowOff / EMU_PER_PIXEL);
		...
		private decimal GetColumnWidth(int col) {
			ExcelWorksheet ws = _drawings.Worksheet;
			return !(ws.GetValueInner(0, col) is ExcelColumn column) ? (decimal)ws.DefaultColWidth : (decimal)ws.Column(col).VisualWidth;
		}
		private double GetRowHeight(int row) {
			...
			if (ws.ExistsValueInner(row, 0, ref o) && o != null)   //Check that the row exists
			{
				...
				return internalRow.Height >= 0 && internalRow.CustomHeight ? internalRow.Height : GetRowHeightFromCellFonts(row, ws);
			} else {
		...
		internal void SetPixelTop(int pixels) {
			...
			var mdw = ws.Workbook.MaxFontWidth;
			var prevPix = 0;
			var pix = (int)(GetRowHeight(1) / 0.75);
			var row = 2;
			while (pix < pixels) {
				prevPix = pix;
				pix += (int)(GetRowHeight(row++) / 0.75);
		...
		internal void SetPixelLeft(int pixels) {
			...
			var mdw = ws.Workbook.MaxFontWidth;
			var prevPix = 0;
			var pix = (int)decimal.Truncate(((256 * GetColumnWidth(1) + decimal.Truncate(128 / (decimal)mdw)) / 256) * mdw);
			var col = 2;
		...
		internal void SetPixelHeight(int pixels) => SetPixelHeight(pixels, STANDARD_DPI);
		internal void SetPixelHeight(int pixels, float dpi) {
			_doNotAdjust = true;
			ExcelWorksheet ws = _drawings.Worksheet;
			//decimal mdw = ws.Workbook.MaxFontWidth;
			pixels = (int)(pixels / (dpi / STANDARD_DPI) + .5);
			var pixOff = pixels - ((int)(GetRowHeight(From.Row + 1) / 0.75) - (int)(From.RowOff / EMU_PER_PIXEL));
			var prevPixOff = pixels;
			var row = From.Row + 1;
			...
			To.Row = row - 1;
			To.RowOff = From.Row == To.Row ? From.RowOff + (pixels) * EMU_PER_PIXEL : prevPixOff * EMU_PER_PIXEL;
			_doNotAdjust = false;
		}
		internal void SetPixelWidth(int pixels) => SetPixelWidth(pixels, STANDARD_DPI);
		internal void SetPixelWidth(int pixels, float dpi) {
			...
			pixels = (int)(pixels / (dpi / STANDARD_DPI) + .5);
			var pixOff = (int)pixels - ((int)decimal.Truncate(((256 * GetColumnWidth(From.Column + 1) + decimal.Truncate(128 / (decimal)mdw)) / 256) * mdw) - From.ColumnOff / EMU_PER_PIXEL);
			var prevPixOff = From.ColumnOff / EMU_PER_PIXEL + (int)pixels;
			var col = From.Column + 2;
		...
		#endregion
		internal virtual void DeleteMe() => TopNode.ParentNode.RemoveChild(TopNode);
