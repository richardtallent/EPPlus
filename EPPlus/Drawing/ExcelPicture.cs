...
namespace OfficeOpenXml.Drawing {
	...
	public sealed class ExcelPicture : ExcelDrawing {
		...
		internal ExcelPicture(ExcelDrawings drawings, XmlNode node) :
			...
			if (picNode != null) {
				...
				if (!string.IsNullOrEmpty(relID)) {
					...
					_hyperlink = HypRel.TargetUri.IsAbsoluteUri
						? new ExcelHyperLink(HypRel.TargetUri.AbsoluteUri)
						: new ExcelHyperLink(HypRel.TargetUri.OriginalString, UriKind.Relative);
...
#else
			var ic = new ImageConverter();
			var img = (byte[])ic.ConvertTo(image, typeof(byte[]));
#endif
		...
		public ImageFormat ImageFormat {
			get => _imageFormat;
			internal set => _imageFormat = value;
		}
