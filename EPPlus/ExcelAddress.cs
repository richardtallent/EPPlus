...
namespace OfficeOpenXml {
	...
	public class ExcelAddressBase : ExcelCellBase {
		...
		public ExcelAddressBase(string address) => SetAddress(address);
		...
		internal eAddressCollition Collide(ExcelAddressBase address, bool ignoreWs = false) {
			...
			if (address._fromRow > _toRow || address._fromCol > _toCol
				...
				return address._fromRow >= _fromRow && address._toRow <= _toRow &&
									   address._fromCol >= _fromCol && address._toCol <= _toCol
					? eAddressCollition.Inside
					: eAddressCollition.Partly;
			}
		}
		internal ExcelAddressBase AddRow(int row, int rows, bool setFixed = false) {
			if (row > _toRow) {
				...
				return row <= _fromRow
					? new ExcelAddressBase((setFixed && _fromRowFixed ? _fromRow : _fromRow + rows), _fromCol, (setFixed && _toRowFixed ? _toRow : _toRow + rows), _toCol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed)
					: new ExcelAddressBase(_fromRow, _fromCol, (setFixed && _toRowFixed ? _toRow : _toRow + rows), _toCol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
			}
		}
		internal ExcelAddressBase DeleteRow(int row, int rows, bool setFixed = false) {
			if (row > _toRow) //After
			{
			  ...
			  {
				return row <= _fromRow
					? new ExcelAddressBase(row, _fromCol, (setFixed && _toRowFixed ? _toRow : _toRow - rows), _toCol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed)
					: new ExcelAddressBase(_fromRow, _fromCol, (setFixed && _toRowFixed ? _toRow : _toRow - rows < row ? row - 1 : _toRow - rows), _toCol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
			}
		}
		internal ExcelAddressBase AddColumn(int col, int cols, bool setFixed = false) {
			if (col > _toCol) {
				...
				return col <= _fromCol
					? new ExcelAddressBase(_fromRow, (setFixed && _fromColFixed ? _fromCol : _fromCol + cols), _toRow, (setFixed && _toColFixed ? _toCol : _toCol + cols), _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed)
					: new ExcelAddressBase(_fromRow, _fromCol, _toRow, (setFixed && _toColFixed ? _toCol : _toCol + cols), _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
			}
		}
		internal ExcelAddressBase DeleteColumn(int col, int cols, bool setFixed = false) {
			if (col > _toCol) //After
			{
			  ...
			  {
				return col <= _fromCol
					? new ExcelAddressBase(_fromRow, col, _toRow, (setFixed && _toColFixed ? _toCol : _toCol - cols), _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed)
					: new ExcelAddressBase(_fromRow, _fromCol, _toRow, (setFixed && _toColFixed ? _toCol : _toCol - cols < col ? col - 1 : _toCol - cols), _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
			}
		}
			...
			} else if (double.TryParse(Address, NumberStyles.Any, CultureInfo.InvariantCulture, out d)) //A double, no valid address
			  {
				...
				if (r1c1 && IsR1C1(Address)) {
					...
					if (SplitAddress(Address, out wb, out ws, out intAddress)) {
						...
						return IsAddress(intAddress)
							? string.IsNullOrEmpty(wb) ? AddressType.InternalAddress : AddressType.ExternalAddress
							: string.IsNullOrEmpty(wb) ? AddressType.InternalName : AddressType.ExternalName;
					} else {
						return AddressType.Invalid;
					}
				}
		...
		private static bool IsValidName(string address) => Regex.IsMatch(address, "[^0-9./*-+,½!\"@#£%&/{}()\\[\\]=?`^~':;<>|][^/*-+,½!\"@#£%&/{}()\\[\\]=?`^~':;<>|]*");
		...
		internal static string GetWorksheetPart(string address, string defaultWorkSheet, ref int endIx) {
			...
			if (ix > 0 && ix < address.Length) {
				if (address[ix] == '\'') {
					...
					return ixEnd > ix ? address.Substring(ix, ixEnd - ix) : defaultWorkSheet;
				}
			} else {
		...
		internal static string GetAddressPart(string address) {
			...
			if (ix < address.Length) {
				return address[ix] == '!' ? address.Substring(ix + 1) : "";
			} else {
				return "";
			}
		...
		internal static void SplitAddress(string fullAddress, out string wb, out string ws, out string address, string defaultWorksheet = "") {
			...
			if (ix < fullAddress.Length) {
				address = fullAddress[ix] == '!' ? fullAddress.Substring(ix + 1) : fullAddress.Substring(ix);
			} else {
				address = "";
			}
		}
	...
	public class ExcelAddress : ExcelAddressBase {
		...
		public ExcelAddress(int fromRow, int fromCol, int toRow, int toColumn)
			: base(fromRow, fromCol, toRow, toColumn) => _ws = "";
	...
	public class ExcelFormulaAddress : ExcelAddressBase {
		...
		internal string GetOffset(int row, int column) {
			...
			var a = GetAddress(fromRow, fromCol, toRow, tocol, _fromRowFixed, _fromColFixed, _toRowFixed, _toColFixed);
			if (Addresses != null) {
