...
namespace OfficeOpenXml {
	...
	public abstract class ExcelCellBase {
		...
		public static string TranslateFromR1C1(string value, int row, int col) => Translate(value, ToAbs, row, col);
		...
		public static string TranslateToR1C1(string value, int row, int col) => Translate(value, ToR1C1, row, col);
		...
		private static string ToR1C1(string part, int row, int col) {
			var shInd = part.IndexOf('!');
			var sh = "";
			if (shInd > 0) {
				sh = part.Substring(0, shInd + 1);
				part = part.Substring(shInd + 1);
			}
			var delim = part.IndexOf(':');
			if (delim > 0) {
				var p1 = ToR1C1_1(part.Substring(0, delim), row, col);
				var p2 = ToR1C1_1(part.Substring(delim + 1), row, col);
		...
		private static string ToAbs(string part, int row, int col) {
			...
			var delim = part.IndexOf(':');
			if (delim > 0) {
				var p1 = ToAbs_1(part.Substring(0, delim), row, col, false);
				var p2 = ToAbs_1(part.Substring(delim + 1), row, col, false);
				return p1.Equals(p2) ? p1 : sh + p1 + ":" + p2;
			} else
				return sh + ToAbs_1(part, row, col, true);
		}
		private static string ToAbs_1(string part, int row, int col, bool isSingle) {
			var check = Utils.ConvertUtil._invariantTextInfo.ToUpper(part);
			// Bug
			var rStart = check.IndexOf("R");
			var cStart = check.IndexOf("C");
			//if (rStart != 0)
			//    return part;
			...
			if (cStart == -1) {
				var RNum = GetRC(part.Substring(1), row, out absoluteRow);
				if (RNum > int.MinValue) {
					var r = GetAddressRow(RNum, absoluteRow);
					return isSingle ? $"{r}:{r}" : $"{r}";
				} else {
					return part;
				}
			}
			if (rStart == -1) {
				var CNum = GetRC(part.Substring(1), col, out absoluteCol);
				if (CNum > int.MinValue) {
					var c = GetAddressCol(CNum, absoluteCol);
					return isSingle ? $"{c}:{c}" : $"{c}";
				} else {
					return part;
				}
			}
			{
				...
				return RNum > int.MinValue && CNum > int.MinValue ? GetAddress(RNum, absoluteRow, CNum, absoluteCol) : part;
			}
		}
		...
		/// <returns></returns>
		private static string GetRCFmt(int v) => (v < 0 ? string.Format("[{0}]", v) : v > 0 ? v.ToString() : "");
		...
		private static int GetRC(string value, int OffsetValue, out bool fixedAddr) {
			...
			{
				fixedAddr = false;
				return int.TryParse(value.Substring(1, value.Length - 2), NumberStyles.Any, CultureInfo.InvariantCulture, out num)
					? OffsetValue + num
					: int.MinValue;
			} else {
				fixedAddr = true;
				return int.TryParse(value, NumberStyles.Any, CultureInfo.InvariantCulture, out num) ? num : int.MinValue;
			}
		}
		#endregion
		...
		#region GetAddress
		public static string GetAddressRow(int Row, bool Absolute = false) => Absolute ? $"${Row}" : $"{Row}";
		public static string GetAddressCol(int Col, bool Absolute = false) {
			var colLetter = GetColumnLetter(Col);
			return Absolute ? $"${colLetter}" : $"{colLetter}";
		}
		...
		/// <returns>The cell address in the format A1</returns>
		public static string GetAddress(int Row, bool AbsoluteRow, int Column, bool AbsoluteCol) => Row < 1 || Row > ExcelPackage.MaxRows || Column < 1 || Column > ExcelPackage.MaxColumns
				? "#REF!"
				: (AbsoluteCol ? "$" : "") + GetColumnLetter(Column) + (AbsoluteRow ? "$" : "") + Row.ToString();
		...
		public static string GetAddress(int Row, int Column, bool Absolute) {
			...
			return Absolute ? "$" + GetColumnLetter(Column) + "$" + Row.ToString() : GetColumnLetter(Column) + Row.ToString();
		}
		...
		public static string GetAddress(int FromRow, int FromColumn, int ToRow, int ToColumn, bool FixedFromRow, bool FixedFromColumn, bool FixedToRow, bool FixedToColumn) {
			if (FromRow == ToRow && FromColumn == ToColumn) {
				...
				if (FromRow == 1 && ToRow == ExcelPackage.MaxRows) {
					...
					return FromColumn == 1 && ToColumn == ExcelPackage.MaxColumns
						? (FixedFromRow ? "$" : "") + FromRow.ToString() + ":" + (FixedToRow ? "$" : "") + ToRow.ToString()
						: GetAddress(FromRow, FixedFromRow, FromColumn, FixedFromColumn) + ":" + GetAddress(ToRow, FixedToRow, ToColumn, FixedToColumn);
				}
			}
		}
		...
		internal static string GetFullAddress(string worksheetName, string address, bool fullRowCol) {
			...
			if (address.IndexOf("!") == -1 || address == "#REF!") {
				if (fullRowCol) {
					...
					address = (a._fromRow == 1 && a._toRow == ExcelPackage.MaxRows) || (a._fromCol == 1 && a._toCol == ExcelPackage.MaxColumns)
						? string.Format("'{0}'!{1}{2}:{3}{4}", worksheetName, ExcelAddress.GetColumnLetter(a._fromCol), a._fromRow, ExcelAddress.GetColumnLetter(a._toCol), a._toRow)
						: GetFullAddress(worksheetName, address, true);
				}
			}
			return address;
		}
