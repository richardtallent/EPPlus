...
namespace OfficeOpenXml {
	...
	public sealed class ExcelPackage : IDisposable {
...
#endif
			if (v != null) {
				if (Boolean.TryParse(v.ToLowerInvariant(), out var value)) {
		...
		/// </summary>
		public Packaging.ZipPackage Package => (_package);
		...
		/// </example>
		/// <returns></returns>
		public byte[] GetAsByteArray() => GetAsByteArray(true);
		...
		public void Load(Stream input) => Load(input, new MemoryStream(), null);
		...
		/// <param name="input">The input.</param>
		/// <param name="Password">The password to decrypt the document</param>
		public void Load(Stream input, string Password) => Load(input, new MemoryStream(), Password);
