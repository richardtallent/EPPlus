...
namespace OfficeOpenXml {
	...
	public class ExcelRangeBase : ExcelAddress, IExcelCell, IDisposable, IEnumerable<ExcelRangeBase>, IEnumerator<ExcelRangeBase> {
		...
		private static void Set_IsRichText(ExcelRangeBase range, object value, int row, int col) => range._worksheet._flags.SetFlagValue(row, col, (bool)value, CellFlags.RichText);
		...
		#endregion
		private void SetToSelectedRange() => Address = _worksheet.View.SelectedRange == "" ? "A1" : _worksheet.View.SelectedRange;
		...
		internal void UpdateAddress(string address) => throw new NotImplementedException();
		...
		public string StyleName {
			get {
				...
				var nsID = xfId <= 0 ? Style.Styles.CellXfs[0].XfId : Style.Styles.CellXfs[xfId].XfId;
			...
			set {
				...
				if (_fromRow == 1 && _toRow == ExcelPackage.MaxRows)    //Full column
				{
					...
					column = c == null ? _worksheet.Column(_fromCol) : (ExcelColumn)c;
		...
		public object Value {
			get {
				if (IsName) {
					...
					return _fromRow == _toRow && _fromCol == _toCol ? _worksheet.GetValue(_fromRow, _fromCol) : GetValueArray();
				}
			}
			set {
				if (IsName) {
			...
			for (var col = addr._fromCol; col <= addr._toCol; col++) {
				for (var row = addr._fromRow; row <= addr._toRow; row++) {
					...
					if (_worksheet.ExistsValueInner(row, col, ref o)) {
						v[row - addr._fromRow, col - addr._fromCol] = _worksheet._flags.GetFlagValue(row, col, CellFlags.RichText) ? GetRichText(row, col).Text : o;
					}
				}
			}
			return v;
		...
		private ExcelAddressBase GetAddressDim(ExcelRangeBase addr) {
			...
			if (addr._fromRow == fromRow && addr._fromCol == fromCol && addr._toRow == toRow && addr._toCol == _toCol) {
				...
				return _fromRow > _toRow || _fromCol > _toCol ? null : new ExcelAddressBase(fromRow, fromCol, toRow, toCol);
			}
		}
		...
		private void SplitFormula(ExcelAddressBase address, int ix) {
			...
			if (collide == eAddressCollition.Partly && (firstCellCollide == eAddressCollition.Inside || firstCellCollide == eAddressCollition.Equal)) //Do we need to split? Only if the functions first row is inside the new range.
			{
				...
				if (fRange._fromCol < address._fromCol) {
					...
					f.Address = fRange._toRow < address._toRow
						? ExcelCellBase.GetAddress(f.StartRow, f.StartCol,
			...
			if (Format.DataTypes == null || Format.DataTypes.Length <= col || Format.DataTypes[col] == eDataTypes.Unknown) {
				...
				switch (Format.DataTypes[col]) {
					case eDataTypes.Number:
						return double.TryParse(v, NumberStyles.Any, Format.Culture, out d) ? d : (object)v;
					case eDataTypes.DateTime:
						return DateTime.TryParse(v, Format.Culture, DateTimeStyles.None, out dt) ? dt : (object)v;
					case eDataTypes.Percent:
						var v2 = v.EndsWith("%") ? v.Substring(0, v.Length - 1) : v;
						return double.TryParse(v2, NumberStyles.Any, Format.Culture, out d) ? d / 100 : (object)v;
		...
		/// <returns>The filled range</returns>
		public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, bool PrintHeaders) => LoadFromCollection<T>(Collection, PrintHeaders, TableStyles.None, BindingFlags.Public | BindingFlags.Instance, null);
		...
		/// <returns>The filled range</returns>
		public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, bool PrintHeaders, TableStyles TableStyle) => LoadFromCollection<T>(Collection, PrintHeaders, TableStyle, BindingFlags.Public | BindingFlags.Instance, null);
			...
			if (Members.Length > 0 && PrintHeaders) {
				foreach (var t in Members) {
					...
					if (t.GetCustomAttributes(typeof(DescriptionAttribute), false).FirstOrDefault() is DescriptionAttribute descriptionAttribute) {
						...
						header = t.GetCustomAttributes(typeof(DisplayNameAttribute), false).FirstOrDefault() is DisplayNameAttribute displayNameAttribute
		...
		public ExcelRangeBase LoadFromText(string Text) => LoadFromText(Text, new ExcelTextFormat());
		...
		public ExcelRangeBase LoadFromText(string Text, ExcelTextFormat Format) {
			...
			var lines = Format.TextQualifier == 0 ? Regex.Split(Text, Format.EOL) : GetLines(Text, Format);
		...
		/// <returns></returns>
		public ExcelRangeBase LoadFromText(FileInfo TextFile) => LoadFromText(File.ReadAllText(TextFile.FullName, Encoding.ASCII));
		...
		/// <param name="Format">Information how to load the text</param>
		/// <returns></returns>
		public ExcelRangeBase LoadFromText(FileInfo TextFile, ExcelTextFormat Format) => LoadFromText(File.ReadAllText(TextFile.FullName, Format.Encoding), Format);
		...
		/// <param name="FirstRowIsHeader">Use the first row as header</param>
		/// <returns></returns>
		public ExcelRangeBase LoadFromText(FileInfo TextFile, ExcelTextFormat Format, TableStyles TableStyle, bool FirstRowIsHeader) => LoadFromText(File.ReadAllText(TextFile.FullName, Format.Encoding), Format, TableStyle, FirstRowIsHeader);
		#endregion
		#region GetValue
		...
		/// </exception>
		public T GetValue<T>() => ConvertUtil.GetTypedCellValue<T>(Value);
		#endregion
		...
		public void Copy(ExcelRangeBase Destination) => Copy(Destination, null);
		...
		public void Copy(ExcelRangeBase Destination, ExcelRangeCopyOptionFlags? excelRangeCopyOptionFlags) {
			...
			if (Worksheet.Dimension == null) {
				...
				range = collideStatus != eAddressCollition.Equal || collideStatus != eAddressCollition.Inside ? Worksheet.Dimension : (this);
			}
		...
		/// Clear all cells
		/// </summary>
		public void Clear() => Delete(this, false);
		...
		internal void Delete(ExcelAddressBase Range, bool shift) {
			...
			fromRow = d != null && Range._fromRow <= d._fromRow && Range._toRow >= d._toRow ? 0 : Range._fromRow;
			fromCol = d != null && Range._fromCol <= d._fromCol && Range._toCol >= d._toCol ? 0 : Range._fromCol;
		...
		/// </summary>
		public ExcelRangeBase Current => new ExcelRangeBase(_worksheet, ExcelAddressBase.GetAddress(cellEnum.Row, cellEnum.Column));
		...
		/// </summary>
		public void Sort() => Sort(new int[] { 0 }, new bool[] { false });
		...
		/// </summary>
		public void Sort(int column, bool descending = false) => Sort(new int[] { column }, new bool[] { descending });
