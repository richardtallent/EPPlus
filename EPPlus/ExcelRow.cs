...
namespace OfficeOpenXml {
	internal class RowInternal {
		...
		internal RowInternal Clone() => new RowInternal() {
			Height = Height,
			Hidden = Hidden,
			Collapsed = Collapsed,
	...
	public class ExcelRow : IRangeID {
		...
		public double Height {
			get {
				var r = (RowInternal)_worksheet.GetValueInner(Row, 0);
				return r == null || r.Height < 0 ? _worksheet.DefaultRowHeight : r.Height;
			}
			set {
				var r = GetRowInternal();
		...
		public bool CustomHeight {
			get {
				var r = (RowInternal)_worksheet.GetValueInner(Row, 0);
				return r == null ? false : r.CustomHeight;
			}
			set {
				var r = GetRowInternal();
		...
		public bool Collapsed {
			get {
				var r = (RowInternal)_worksheet.GetValueInner(Row, 0);
				return r == null ? false : r.Collapsed;
			}
			set {
				var r = GetRowInternal();
				r.Collapsed = value;
		...
		public int OutlineLevel {
			get {
				var r = (RowInternal)_worksheet.GetValueInner(Row, 0);
				return r == null ? 0 : r.OutlineLevel;
			}
			set {
				var r = GetRowInternal();
		...
		public bool Phonetic {
			get {
				var r = (RowInternal)_worksheet.GetValueInner(Row, 0);
				return r == null ? false : r.Phonetic;
			}
			set {
				var r = GetRowInternal();
				r.Phonetic = value;
		...
		public bool PageBreak {
			get {
				var r = (RowInternal)_worksheet.GetValueInner(Row, 0);
				return r == null ? false : r.PageBreak;
			}
			set {
				var r = GetRowInternal();
				r.PageBreak = value;
