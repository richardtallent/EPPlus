	...
	public sealed class ExcelStyles : XmlHelper {
		...
		private void LoadFromDocument() {
			...
			foreach (XmlNode n in fillNode) {
				ExcelFillXml f = n.FirstChild != null && n.FirstChild.LocalName == "gradientFill"
					? new ExcelGradientFillXml(_nameSpaceManager, n)
					: new ExcelFillXml(_nameSpaceManager, n);
				Fills.Add(f.Id, f);
			}
		...
		private void AddNamedStyle(int id, XmlNode styleXfsNode, XmlNode cellXfsNode, ExcelNamedStyleXml style) {
			...
			style.XfId = style.XfId >= 0 ? CellXfs[style.XfId].newID : 0;
		}
		...
		#endregion
		internal int CloneStyle(ExcelStyles style, int styleID) => CloneStyle(style, styleID, false, false);
		internal int CloneStyle(ExcelStyles style, int styleID, bool isNamedStyle) => CloneStyle(style, styleID, isNamedStyle, false);
		internal int CloneStyle(ExcelStyles style, int styleID, bool isNamedStyle, bool allwaysAddCellXfs) {
			ExcelXfs xfs;
			lock (style) {
				xfs = isNamedStyle ? style.CellStyleXfs[styleID] : style.CellXfs[styleID];
