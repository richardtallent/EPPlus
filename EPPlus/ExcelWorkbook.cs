	...
	public sealed class ExcelWorkbook : XmlHelper, IDisposable {
		...
		private void GetSharedStrings() {
			if (_package.Package.PartExists(SharedStringsUri)) {
				...
				var nl = xml.SelectNodes("//d:sst/d:si", NameSpaceManager);
		...
		internal void GetDefinedNames() {
			var nl = WorkbookXml.SelectNodes("//d:definedNames/d:definedName", NameSpaceManager);
			if (nl != null) {
				foreach (XmlElement elem in nl) {
					...
					if (addressType == ExcelAddressBase.AddressType.Invalid
						...
						namedRange = nameWorksheet == null
							...
							: nameWorksheet.Names.Add(elem.GetAttribute("name"), range);
						...
						var addr = new ExcelAddress(fullAddress, _package, null);
						if (localSheetID > -1) {
							namedRange = string.IsNullOrEmpty(addr._ws)
								? Worksheets[localSheetID + _package._worksheetAdd].Names.Add(elem.GetAttribute("name"), new ExcelRangeBase(this, Worksheets[localSheetID + _package._worksheetAdd], fullAddress, false))
								: Worksheets[localSheetID + _package._worksheetAdd].Names.Add(elem.GetAttribute("name"), new ExcelRangeBase(this, Worksheets[addr._ws], fullAddress, false));
						} else {
		...
		public ExcelWorksheets Worksheets {
			get {
				if (_worksheets == null) {
					...
					if (sheetsNode == null) sheetsNode = CreateNode("d:sheets");
		...
		internal FormulaParser FormulaParser {
			get {
				if (_formulaParser == null) _formulaParser = new FormulaParser(new EpplusExcelDataProvider(_package));
		...
		public FormulaParserManager FormulaParserManager {
			get {
				if (_parserManager == null) _parserManager = new FormulaParserManager(FormulaParser);
		...
		public decimal MaxFontWidth {
				...
				if (ix >= 0) {
					if (_standardFontWidth == decimal.MinValue || _fontID != Styles.NamedStyles[ix].Style.Font.Id) {
						...
						try {
							...
							// Error, Font missing and Calibri removed in dictionary
			...
			set => _standardFontWidth = value;
		}

		internal static decimal GetWidthPixels(string fontName, float fontSize) {
			var font = FontSize.FontHeights.ContainsKey(fontName) ? FontSize.FontHeights[fontName] : FontSize.FontHeights["Calibri"];
			if (font.ContainsKey(fontSize)) {
				...
				return min == max
					? Convert.ToDecimal(font[min].Width)
					: Convert.ToDecimal(font[min].Height + (font[max].Height - font[min].Height) * ((fontSize - min) / (max - min)));
			}
		}
		...
		/// </summary>
		internal Packaging.ZipPackagePart Part => (_package.Package.GetPart(WorkbookUri));
		...
		private void CreateWorkbookXml(XmlNamespaceManager namespaceManager) {
			if (_package.Package.PartExists(WorkbookUri))
				...
				var stream = new StreamWriter(partWorkbook.GetStream(FileMode.Create, FileAccess.Write));
			...
			get {
				if (_stylesXml == null) {
					if (_package.Package.PartExists(StylesUri))
						...
						var xml = new StringBuilder("<styleSheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\">");
						xml.Append("<numFmts />");
		...
		public ExcelCalcMode CalcMode {
			get {
				var calcMode = GetXmlNodeString(CALC_MODE_PATH);
				switch (calcMode) {
					case "autoNoTable":
		...
		public bool FullCalcOnLoad {
			get => GetXmlNodeBool(FULL_CALC_ON_LOAD_PATH);
			set => SetXmlNodeBool(FULL_CALC_ON_LOAD_PATH, value);
		}
		#endregion
		...
		private void DeleteCalcChain() {
			...
			var uriCalcChain = new Uri("/xl/calcChain.xml", UriKind.Relative);
		...
		private void SaveSharedStringHandler(ZipOutputStream stream, CompressionLevel compressionLevel, string fileName) {
			...
			foreach (var t in _sharedStrings.Keys) {
			...
			if (name.IsName) {
				if (string.IsNullOrEmpty(name.NameFormula)) {
					if ((TypeCompat.IsPrimitive(name.NameValue) || name.NameValue is double || name.NameValue is decimal)) {
						...
						elem.InnerText = name.NameValue is DateTime
		...
		internal bool ExistsTableName(string Name) {
			foreach (var ws in Worksheets) {
				if (ws.Tables._tableNames.ContainsKey(Name)) return true;
			}
			return false;
		}
		...
		internal bool ExistsPivotTableName(string Name) {
			foreach (var ws in Worksheets) {
				if (ws.PivotTables._pivotTableNames.ContainsKey(Name)) return true;
			}
			return false;
		}
			...
			if (nl != null) {
				foreach (XmlElement elem in nl) {
					...
					if (xmlExtRef.SelectSingleNode("//d:externalBook", NameSpaceManager) is XmlElement book) {
						...
						if (rel_extRef != null) _externalReferences.Add(rel_extRef.TargetUri.OriginalString);
					}
				}
			}
		}
		...
		internal void ReadAllTables() {
			...
			_nextPivotTableID = 1;
			foreach (var ws in Worksheets) {
				// Fixes 15273.Chartsheets should be ignored.
				if (!(ws is ExcelChartsheet)) {
					foreach (var tbl in ws.Tables) {
						if (tbl.Id >= _nextTableID) _nextTableID = tbl.Id + 1;
					}
					foreach (var pt in ws.PivotTables) {
						if (pt.CacheID >= _nextPivotTableID) _nextPivotTableID = pt.CacheID + 1;
					}
				}
			}
		}
