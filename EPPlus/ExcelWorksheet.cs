...
namespace OfficeOpenXml {
	...
	public class ExcelChartsheet : ExcelWorksheet {
		...
		public ExcelChartsheet(XmlNamespaceManager ns, ExcelPackage pck, string relID, Uri uriWorksheet, string sheetName, int sheetID, int positionID, eWorkSheetHidden hidden, eChartType chartType, ExcelPivotTable pivotTableSource) :
			base(ns, pck, relID, uriWorksheet, sheetName, sheetID, positionID, hidden) => this.Drawings.AddChart("Chart 1", chartType, pivotTableSource);
		...
		public ExcelChart Chart => (ExcelChart)Drawings[0];
	}
	...
	public class ExcelWorksheet : XmlHelper, IEqualityComparer<ExcelWorksheet>, IDisposable {
		internal class Formulas {
			public Formulas(ISourceCodeTokenizer tokenizer) => _tokenizer = tokenizer;
		...
		public class MergeCellsCollection : IEnumerable<string> {
			...
			public string this[int row, int column] {
				get {
					var ix = -1;
					return _cells.Exists(row, column, ref ix) && ix >= 0 && ix < List.Count ? List[ix] : null;
				}
			}
			...
			private bool Validate(ExcelAddressBase address) {
				...
				if (_cells.Exists(address._fromRow, address._fromCol, ref ix)) {
					return ix >= 0 && ix < _list.Count && _list[ix] != null && address.Address == _list[ix];
				}
			...
			internal void Remove(string Item) => _list.Remove(Item);
		...
		/// </summary>
		internal Packaging.ZipPackagePart Part => (_package.Package.GetPart(WorksheetUri));
		...
		/// </summary>
		internal int PositionID { get => (_positionID); set => _positionID = value; }
		...
		public ExcelAddressBase AutoFilterAddress {
			get {
				CheckSheetType();
				var address = GetXmlNodeString("d:autoFilter/@ref");
		...
		public eWorkSheetHidden Hidden {
			get {
				var state = _package.Workbook.GetXmlNodeString(string.Format("d:sheets/d:sheet[@sheetId={0}]/@state", _sheetID));
				if (state == "hidden") {
		...
		public double DefaultRowHeight {
			...
			set {
				...
				if (double.IsNaN(value)) {
					...
					var defHeight = GetRowHeightFromNormalStyle();
					CustomHeight = true;
				}
			}
		}
		...
		public bool CustomHeight {
			get => GetXmlNodeBool("d:sheetFormatPr/@customHeight");
			set => SetXmlNodeBool("d:sheetFormatPr/@customHeight", value);
		}
		...
		public double DefaultColWidth {
			get {
				CheckSheetType();
				var ret = GetXmlNodeDouble("d:sheetFormatPr/@defaultColWidth");
				if (double.IsNaN(ret)) {
		...
		public Color TabColor {
			...
			set => SetXmlNodeString(tabColorPath, value.ToArgb().ToString("X"));
		}
		const string codeModuleNamePath = "d:sheetPr/@codeName";
		internal string CodeModuleName {
			get => GetXmlNodeString(codeModuleNamePath);
		...
		private void CreateXml() {
			...
			var doAdjust = _package.DoAdjustDrawings;
			...
			var end = stream.Position - nextElementLength;
			LoadMergeCells(xr);
			LoadHyperLinks(xr);
		...
		private string GetWorkSheetXml(Stream stream, long start, long end, out Encoding encoding) {
			...
			do {
				var size = stream.Length < BLOCKSIZE ? (int)stream.Length : BLOCKSIZE;
				block = new char[size];
			...
			if (!startmMatch.Success) //Not found
			{
				...
				var s = sb.ToString();
				var xml = s.Substring(0, startmMatch.Index);
			...
			if (ReadUntil(xr, "cols", "sheetData")) {
				while (xr.Read()) {
					...
					if (xr.NodeType == XmlNodeType.Element) {
						...
						var col = new ExcelColumn(this, min);
		...
		private void LoadHyperLinks(XmlReader xr) {
			...
			while (xr.Read()) {
				if (xr.LocalName == "hyperlink") {
					...
					var tt = xr.GetAttribute("tooltip");
			...
			while (!xr.EOF) {
				...
				if (xr.LocalName == "row") {
					...
					if (t == null) {
					  ...
					  {
						var aAddress = xr.GetAttribute("ref");
		...
		private void UpdateMergedCells(StreamWriter sw) {
			sw.Write("<mergeCells>");
			foreach (var address in _mergedCells) {
		...
		/// <returns></returns>
		private RowInternal AddRow(XmlReader xr, int row) => new RowInternal() {
		...
		private void SetValueFromXml(XmlReader xr, string type, int styleID, int row, int col) {
			...
			if (type == "s") {
				var ix = xr.ReadElementContentAsInt();
		...
		public ExcelColumn Column(int col) {
			...
			if (GetValueInner(0, col) is ExcelColumn column) {

				if (column.ColumnMin != column.ColumnMax) {
					var maxCol = column.ColumnMax;
		...
		internal ExcelColumn CopyColumn(ExcelColumn c, int col, int maxCol) {
			var newC = new ExcelColumn(this, col);
		...
		/// </summary>
		public void Select() => View.TabSelected = true;//Select(Address, true);
		...
		public void Select(string Address) => Select(Address, true);
		...
		public void Select(ExcelAddress Address, bool SelectSheet) {
			...
			var selAddress = ExcelCellBase.GetAddress(Address.Start.Row, Address.Start.Column) + ":" + ExcelCellBase.GetAddress(Address.End.Row, Address.End.Column);
		...
		public void InsertRow(int rowFrom, int rows) => InsertRow(rowFrom, rows, 0);
		...
		public void InsertColumn(int columnFrom, int columns) => InsertColumn(columnFrom, columns, 0);
		///<summary>
		...
		private void FixMergedCellsRow(int row, int rows, bool delete) {
			...
			for (var i = 0; i < _mergedCells.Count; i++) {
			...
			for (var i = removeIndex.Count - 1; i >= 0; i--) {
		...
		private void FixSharedFormulasRows(int position, int rows) {
			var added = new List<Formulas>();
			var deleted = new List<Formulas>();
			...
			foreach (var id in _sharedFormulas.Keys) {
				...
				if (position >= fromRow && position + (Math.Abs(rows)) <= toRow) //Insert/delete is whithin the share formula address
				{
					if (rows > 0) //Insert
					{
						...
						if (toRow != fromRow) {
							var newF = new Formulas(SourceCodeTokenizer.Default);
						...
						f.Address = fromRow - rows < toRow
							? ExcelCellBase.GetAddress(fromRow, fromCol, toRow + rows, toCol)
							: ExcelCellBase.GetAddress(fromRow, fromCol) + ":" + ExcelCellBase.GetAddress(toRow + rows, toCol);
					}
		...
		private void UpdateSharedFormulaRow(ref Formulas formula, int startRow, int rows, ref List<Formulas> newFormulas) {
			...
			var prevFormualR1C1 = formualR1C1;
		...
		public void DeleteRow(int row) => DeleteRow(row, 1);
		...
		public void DeleteColumn(int column) => DeleteColumn(column, 1);
		...
		public void DeleteColumn(int columnFrom, int columns) {
			...
			lock (this) {
				if (!(GetValueInner(0, columnFrom) is ExcelColumn col)) {
					var r = 0;
					var c = columnFrom;
					if (_values.PrevCell(ref r, ref c)) {
		...
		public void DeleteRow(int rowFrom, int rows, bool shiftOtherRowsUp) => DeleteRow(rowFrom, rows);
		#endregion
		...
		public object GetValue(int Row, int Column) {
			...
			if (v != null) {
				...
				return _flags.GetFlagValue(Row, Column, CellFlags.RichText) ? (object)Cells[Row, Column].RichText.Text : v;
			} else {
				return null;
			}
		}
		...
		public T GetValue<T>(int Row, int Column) {
			...
			//var cell=((ExcelCell)_cells[cellID]);
			return _flags.GetFlagValue(Row, Column, CellFlags.RichText)
				? (T)(object)Cells[Row, Column].RichText.Text
				: ConvertUtil.GetTypedCellValue<T>(v);
		}
		...
		private void DeletePrinterSettings() {
			...
			var attr = (XmlAttribute)WorksheetXml.SelectSingleNode("//d:pageSetup/@r:id", NameSpaceManager);
			if (attr != null) {
		...
		private void SaveTables() {
			foreach (var tbl in Tables) {
				if (tbl.ShowHeader || tbl.ShowTotal) {
					var colNum = tbl.Address._fromCol;
					...
					foreach (var col in tbl.Columns) {
						...
						if (!string.IsNullOrEmpty(col.CalculatedColumnFormula)) {
							var fromRow = tbl.ShowHeader ? tbl.Address._fromRow + 1 : tbl.Address._fromRow;
							var toRow = tbl.ShowTotal ? tbl.Address._toRow - 1 : tbl.Address._toRow;
							var r1c1Formula = ExcelCellBase.TranslateToR1C1(col.CalculatedColumnFormula, fromRow, colNum);
							var needsTranslation = r1c1Formula != col.CalculatedColumnFormula;
					...
					if (fields != null) {
						...
						foreach (var df in pt.DataFields) {
							if (string.IsNullOrEmpty(df.Name)) {
								var name = df.Function == DataFieldFunctions.None ? df.Field.Name : df.Function.ToString() + " of " + df.Field.Name;
		...
		private static string GetTotalFunction(ExcelTableColumn col, string FunctionNum) {
			var escapedColumn = Regex.Replace(col.Name, @"[\[\]#']", new MatchEvaluator(m => "'" + m.Value));
		...
		private void SaveXml(Stream stream) {
			...
			var sw = new StreamWriter(stream, System.Text.Encoding.UTF8, 65536);
			if (this is ExcelChartsheet) {
		...
		private void UpdateRowCellData(StreamWriter sw) {
			...
			while (cse.Next()) {
				if (cse.Column > 0) {
					...
					var styleID = cellXfs[(val._styleId == 0 ? GetStyleIdDefaultWithMemo(cse.Row, cse.Column) : val._styleId)].newID;
					...
					if (formula is int) {
						...
						if (v == null && styleID > 0) {
							...
							if (v is System.Collections.IEnumerable enumerableResult && !(v is string)) {
								...
								v = enumerator.MoveNext() && enumerator.Current != null ? enumerator.Current : string.Empty;
							}
		...
		private string GetCellType(object v, bool allowStr = false) {
			if (v is bool) {
				...
				return allowStr && v != null && !(TypeCompat.IsPrimitive(v) || v is double || v is decimal || v is DateTime || v is TimeSpan)
					? " t=\"str\""
					: "";
			}
		}
			...
			try {
				if (v is DateTime) {
					...
					if (v is double && double.IsNaN((double)v)) {
						...
						s = v is double && double.IsInfinity((double)v)
							? "#NUM!"
							: Convert.ToDouble(v, CultureInfo.InvariantCulture).ToString("R15", CultureInfo.InvariantCulture);
					}
				} else {
					s = v.ToString();
				}
		...
		private void UpdateHyperLinks(StreamWriter sw) {
			var hyps = new Dictionary<string, string>();
			...
			while (cse.Next()) {
				...
				if (uri is ExcelHyperLink && !string.IsNullOrEmpty((uri as ExcelHyperLink).ReferenceAddress)) {
					...
					string id;
					Uri hyp = uri is ExcelHyperLink ? ((ExcelHyperLink)uri).OriginalUri : uri;
		...
		internal void SetHFLegacyDrawingRel(string relID) => SetXmlNodeString("d:legacyDrawingHF/@r:id", relID);
		...
		internal string GetFormulaR1C1(int row, int col) {
			...
			if (v is int) {
				...
				return v != null ? ExcelCellBase.TranslateToR1C1(v.ToString(), row, col) : "";
			}
		}
		...
		internal ExcelColumn GetColumn(int column) {
			...
			if (c == null) {
				...
				if (_values.PrevCell(ref row, ref col)) {
					...
					return c != null && c.ColumnMax >= column ? c : null;
				}
			}
			return c;
		...
		internal object GetValueInner(int row, int col) => _values.GetValue(row, col)._value;
		...
		internal int GetStyleInner(int row, int col) => _values.GetValue(row, col)._styleId;
		...
		internal void SetStyleInner(int row, int col, int styleId) => _values.SetValueSpecial(row, col, (CellStore<ExcelCoreValue>.SetValueDelegate)SetStyleInnerUpdate, styleId);
		void SetStyleInnerUpdate(List<ExcelCoreValue> list, int index, object styleId) => list[index] = new ExcelCoreValue { _value = list[index]._value, _styleId = (int)styleId };
		...
		internal bool ExistsValueInner(int row, int col) => (_values.GetValue(row, col)._value != null);
		...
		/// <param name="col">column</param>
		/// <returns>is exists</returns>
		internal bool ExistsStyleInner(int row, int col) => (_values.GetValue(row, col)._styleId != 0);
