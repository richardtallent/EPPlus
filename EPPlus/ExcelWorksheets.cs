...
namespace OfficeOpenXml {
	...
	public class ExcelWorksheets : XmlHelper, IEnumerable<ExcelWorksheet>, IDisposable {
		...
		public IEnumerator<ExcelWorksheet> GetEnumerator() => (_worksheets.Values.GetEnumerator());
		#region IEnumerable Members

		IEnumerator IEnumerable.GetEnumerator() => (_worksheets.Values.GetEnumerator());
		...
		private ExcelWorksheet AddSheet(string Name, bool isChart, eChartType? chartType, ExcelPivotTable pivotTableSource = null) {
			lock (_worksheets) {
				...
				ExcelWorksheet worksheet = isChart
					? new ExcelChartsheet(_namespaceManager, _pck, rel, uriWorksheet, Name, sheetID, positionID, eWorkSheetHidden.Visible, (eChartType)chartType, pivotTableSource)
					: new ExcelWorksheet(_namespaceManager, _pck, rel, uriWorksheet, Name, sheetID, positionID, eWorkSheetHidden.Visible);
		...
		/// <returns></returns>
		public ExcelChartsheet AddChart(string Name, eChartType chartType) => (ExcelChartsheet)AddSheet(Name, true, chartType, null);
		...
		private void CopySheetNames(ExcelWorksheet Copy, ExcelWorksheet added) {
			foreach (var name in Copy.Names) {
				...
				if (!name.IsName) {
					newName = name.WorkSheet == Copy.Name
						? added.Names.Add(name.Name, added.Cells[name.FirstAddress])
						: added.Names.Add(name.Name, added.Workbook.Worksheets[name.WorkSheet].Cells[name.FirstAddress]);
				} else {
					newName = !string.IsNullOrEmpty(name.NameFormula)
						? added.Names.AddFormula(name.Name, name.Formula)
						: added.Names.AddValue(name.Name, name.Value);
				}
		...
		private void CopyComment(ExcelWorksheet Copy, ExcelWorksheet workSheet) {
			...
			//Add the relationship ID to the worksheet xml.
			if (!(workSheet.WorksheetXml.SelectSingleNode("//d:legacyDrawing", _namespaceManager) is XmlElement e)) {
		...
		private void CopyVmlDrawing(ExcelWorksheet origSheet, ExcelWorksheet newSheet) {
			...
			if (!(newSheet.WorksheetXml.SelectSingleNode("//d:legacyDrawing", _namespaceManager) is XmlElement e)) {
		...
		private void GetSheetURI(ref string Name, out int sheetID, out Uri uriWorksheet, bool isChart) {
			...
			do {
				uriWorksheet = isChart
					? new Uri("/xl/chartsheets/chartsheet" + uriId + ".xml", UriKind.Relative)
					: new Uri("/xl/worksheets/sheet" + uriId + ".xml", UriKind.Relative);
		...
		public void MoveBefore(string sourceName, string targetName) => Move(sourceName, targetName, false);
		...
		public void MoveBefore(int sourcePositionId, int targetPositionId) => Move(sourcePositionId, targetPositionId, false);
		...
		/// <param name="sourceName">The name of the source worksheet</param>
		/// <param name="targetName">The name of the target worksheet</param>
		public void MoveAfter(string sourceName, string targetName) => Move(sourceName, targetName, true);
		...
		/// <param name="sourcePositionId">The id of the source worksheet</param>
		/// <param name="targetPositionId">The id of the target worksheet</param>
		public void MoveAfter(int sourcePositionId, int targetPositionId) => Move(sourcePositionId, targetPositionId, true);
		...
		public void MoveToStart(int sourcePositionId) => Move(sourcePositionId, _pck._worksheetAdd, false);
		...
		/// </summary>
		/// <param name="sourcePositionId"></param>
		public void MoveToEnd(int sourcePositionId) => Move(sourcePositionId, _worksheets.Count + (_pck._worksheetAdd - 1), true);
