		...
		private static void GetChain(DependencyChain depChain, ILexer lexer, ExcelRangeBase Range, ExcelCalculationOption options) {
			...
			while (fs.Next()) {
				...
				if (!depChain.index.ContainsKey(id)) {
					...
					f.Formula = fs.Value is int ? ws._sharedFormulas[(int)fs.Value].GetFormula(fs.Row, fs.Column, ws.Name) : fs.Value.ToString();
					if (!string.IsNullOrEmpty(f.Formula)) {
				...
				if (t.TokenType == TokenType.ExcelAddress) {
					...
					if (!string.IsNullOrEmpty(adrWs)) {
						...
						if (f.ws.Names.ContainsKey(t.Value)) {
							...
							name = wb.Names.ContainsKey(adrName) ? wb.Names[adrName] : null;
						}
...
iterateCells:
			...
			while (f.iterator != null && f.iterator.Next()) {
				...
				if (!depChain.index.ContainsKey(id)) {
					...
					rf.Formula = f.iterator.Value is int ? f.ws._sharedFormulas[(int)v].GetFormula(f.iterator.Row, f.iterator.Column, ws.Name) : v.ToString();
					rf.ws = f.ws;
