...
namespace OfficeOpenXml.FormulaParsing {
	public class EpplusExcelDataProvider : ExcelDataProvider {
		public class RangeInfo : IRangeInfo {
			...
			public int GetNCells() => ((_toRow - _fromRow) + 1) * ((_toCol - _fromCol) + 1);
			...
			public void Reset() => _values.Init();
			...
			public object GetValue(int row, int col) => _ws.GetValue(row, col);

			public object GetOffset(int rowOffset, int colOffset) => _values.Row < _fromRow || _values.Column < _fromCol
					? _ws.GetValue(_fromRow + rowOffset, _fromCol + colOffset)
					: _ws.GetValue(_values.Row + rowOffset, _values.Column + colOffset);
		}
		...
		public override ExcelNamedRangeCollection GetWorksheetNames(string worksheet) {
			...
			return ws != null ? ws.Names : null;
		}

		public override ExcelNamedRangeCollection GetWorkbookNameValues() => _package.Workbook.Names;
		...
		public override INameInfo GetName(string worksheet, string name) {
			...
			if (_names.ContainsKey(id)) {
				...
				ni.Value = nameItem._fromRow > 0
					? new RangeInfo(nameItem.Worksheet ?? ws, nameItem._fromRow, nameItem._fromCol, nameItem._toRow, nameItem._toCol)
					: nameItem.Value;
				_names.Add(id, ni);
				return ni;
			}
		...
		public object GetValue(int row, int column) => _currentWorksheet.GetValueInner(row, column);

		public bool IsMerged(int row, int column) =>
			//return _currentWorksheet._flags.GetFlagValue(row, column, CellFlags.Merged);
			_currentWorksheet.MergedCells[row, column] != null;

		public bool IsHidden(int row, int column) => _currentWorksheet.Column(column).Hidden || _currentWorksheet.Column(column).Width == 0 ||
		...
		private void SetCurrentWorksheet(string worksheetName) => _currentWorksheet = !string.IsNullOrEmpty(worksheetName) ? _package.Workbook.Worksheets[worksheetName] : _package.Workbook.Worksheets.First();
		...
		public override void Dispose() => _package.Dispose();
		...
		public override List<LexicalAnalysis.Token> GetRangeFormulaTokens(string worksheetName, int row, int column) => _package.Workbook.Worksheets[worksheetName]._formulaTokens.GetValue(row, column);
		...
		public override void Reset() => _names = new Dictionary<ulong, INameInfo>(); //Reset name cache.            
