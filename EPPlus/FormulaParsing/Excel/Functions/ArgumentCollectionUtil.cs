...
namespace OfficeOpenXml.FormulaParsing.Excel.Functions {
	public class ArgumentCollectionUtil {
		...
		public virtual IEnumerable<ExcelDoubleCellValue> ArgsToDoubleEnumerable(bool ignoreHidden, bool ignoreErrors, IEnumerable<FunctionArgument> arguments, ParsingContext context) => _doubleEnumerableArgConverter.ConvertArgs(ignoreHidden, ignoreErrors, arguments, context);

		public virtual IEnumerable<object> ArgsToObjectEnumerable(bool ignoreHidden,
																  IEnumerable<FunctionArgument> arguments,
																  ParsingContext context) => _objectEnumerableArgConverter.ConvertArgs(ignoreHidden, arguments, context);
		...
		public virtual double CalculateCollection(IEnumerable<FunctionArgument> collection, double result, Func<FunctionArgument, double, double> action) {
			foreach (var item in collection) {
				result = item.Value is IEnumerable<FunctionArgument>
					? CalculateCollection((IEnumerable<FunctionArgument>)item.Value, result, action)
					: action(item, result);
			}
			return result;
		}
