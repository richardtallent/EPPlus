...
namespace OfficeOpenXml.FormulaParsing.Excel.Functions.Math {
	public class SumIfs : MultipleRangeCriteriasFunction {
		public override CompileResult Execute(IEnumerable<FunctionArgument> arguments, ParsingContext context) {
			...
			List<double> sumRange = valueRange != null
				? ArgsToDoubleEnumerableZeroPadded(true, valueRange, context).ToList()
				: ArgsToDoubleEnumerable(true, new List<FunctionArgument> { functionArguments[0] }, context).Select(x => (double)x).ToList();
