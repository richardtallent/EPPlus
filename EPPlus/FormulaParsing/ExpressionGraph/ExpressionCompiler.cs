...
namespace OfficeOpenXml.FormulaParsing.ExpressionGraph {
	public class ExpressionCompiler : IExpressionCompiler {
		...
		private CompileResult PerformCompilation(string worksheet = "", int row = -1, int column = -1) {
			...
			return _expressions.Any() ? compiledExpressions.First().Compile() : CompileResult.Empty;
		}
		...
		private int FindLowestPrecedence() => _expressions.Where(x => x.Operator != null).Min(x => x.Operator.Precedence);
