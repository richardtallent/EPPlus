...
namespace OfficeOpenXml.FormulaParsing {
	public class FormulaParser : IDisposable {
		...
		public virtual object Parse(string formula, string address) => Parse(formula, _parsingContext.RangeAddressFactory.Create(address));

		public virtual object Parse(string formula) => Parse(formula, RangeAddress.Empty);
		...
		public virtual object ParseAt(string worksheetName, int row, int col) {
			...
			return string.IsNullOrEmpty(f)
				? _excelDataProvider.GetRangeValue(worksheetName, row, col)
				: Parse(f, _parsingContext.RangeAddressFactory.Create(worksheetName, col, row));
