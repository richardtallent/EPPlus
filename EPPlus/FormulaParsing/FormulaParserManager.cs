...
namespace OfficeOpenXml.FormulaParsing {
	...
	public class FormulaParserManager {
		...
		public void LoadFunctionModule(IFunctionModule module) => _parser.Configure(x => x.FunctionRepository.LoadModule(module));
		...
		public void AddOrReplaceFunction(string functionName, ExcelFunction functionImpl) => _parser.Configure(x => x.FunctionRepository.AddOrReplaceFunction(functionName, functionImpl));
		...
		/// <returns></returns>
		public object Parse(string formula) => _parser.Parse(formula);
		...
		public void AttachLogger(IFormulaParserLogger logger) => _parser.Configure(c => c.AttachLogger(logger));

		/// <summary>
		/// Attaches a logger to the formula parser that produces output to the supplied logfile.
		/// </summary>
		/// <param name="logfile"></param>
		public void AttachLogger(FileInfo logfile) => _parser.Configure(c => c.AttachLogger(LoggerFactory.CreateTextFileLogger(logfile)));

		/// <summary>
		/// Detaches any attached logger from the formula parser.
		/// </summary>
		public void DetachLogger() => _parser.Configure(c => c.DetachLogger());
	}
}
