...
namespace OfficeOpenXml.Packaging.Ionic {
	...
	internal partial class NameCriterion : SelectionCriterion {
		...
		internal virtual string MatchingFileSpec {
			set {
				// workitem 8245
				_MatchingFileSpec = Directory.Exists(value) ? ".\\" + value + "\\*.*" : value;
	...
	internal partial class AttributesCriterion : SelectionCriterion {
		...
		private bool _EvaluateOne(FileAttributes fileAttrs, FileAttributes criterionAttrs) {
			var result = (_Attributes & criterionAttrs) == criterionAttrs ? (fileAttrs & criterionAttrs) == criterionAttrs : true;
			return result;
		}
	...
	internal partial class FileSelector {
		...
		public String SelectionCriteria {
			...
			set {
				...
				else _Criterion = value.Trim() == "" ? null : _ParseCriterion(value);
			}
		}
			...
			for (var i = 0; i < tokens.Length; i++) {
				...
				switch (tok1) {
					...
					case "size":
						...
						else sz = v.EndsWith("GB", StringComparison.OrdinalIgnoreCase)
							? Int64.Parse(v.Substring(0, v.Length - 2)) * 1024 * 1024 * 1024
							: Int64.Parse(tokens[i + 2]);
