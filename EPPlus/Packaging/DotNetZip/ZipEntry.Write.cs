...
namespace OfficeOpenXml.Packaging.Ionic.Zip {
	internal partial class ZipEntry {
		internal void WriteCentralDirectoryEntry(Stream s) {
			...
			// Disk number start
			var segmented = (this._container.ZipFile != null) &&
		...
		internal void WriteHeader(Stream s, int cycle) {
			...
			if (zss != null) {
				...
				_future_ROLH = requiredSegment != zss.CurrentSegment ? 0 : zss.Position;
		...
		private long SetInputAndFigureFileLength(ref Stream input) {
			...
			if (this._Source == ZipEntrySource.Stream) {
				...
				try { fileLength = this._sourceStream.Length; } catch (NotSupportedException) { }
			} else if (this._Source == ZipEntrySource.ZipFile) {
				// we are "re-streaming" the zip entry.
				var pwd = (_Encryption_FromZipFile == EncryptionAlgorithm.None) ? null : (this._Password ?? this._container.Password);
			...
			do {
				try {
					...
					do {
						...
						else readAgain = !s.CanSeek ? false : WantReadAgain();
		...
		private void CopyThroughWithRecompute(Stream outstream) {
			...
			if (!this.FileName.EndsWith("/")) {
				...
				var remaining = this._CompressedSize;
		...
		private void CopyThroughWithNoChange(Stream outstream) {
			...
			var remaining = this._TotalEntrySize;
			while (remaining > 0) {
