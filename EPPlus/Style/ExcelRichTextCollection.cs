...
namespace OfficeOpenXml.Style {
	...
	public class ExcelRichTextCollection : XmlHelper, IEnumerable<ExcelRichText> {
		...
		/// <returns></returns>
		public ExcelRichText Add(string Text) => Insert(_list.Count, Text);
		...
		public ExcelRichText Insert(int index, string text) {
			ConvertRichtext();
			XmlDocument doc = TopNode is XmlDocument ? TopNode as XmlDocument : TopNode.OwnerDocument;
			...
			if (_list.Count > 0) {
				...
				rt.Color = prevItem.Color.IsEmpty ? Color.Black : prevItem.Color;
		...
		internal void UpdateCells() => _cells.SetValueRichText(TopNode.InnerXml);
		...
		IEnumerator<ExcelRichText> IEnumerable<ExcelRichText>.GetEnumerator() => _list.Select(x => { x.SetCallback(UpdateCells); return x; }).GetEnumerator();

		#endregion

		#region IEnumerable Members

		System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator() => _list.Select(x => { x.SetCallback(UpdateCells); return x; }).GetEnumerator();
