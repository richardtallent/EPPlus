	...
	public sealed class ExcelFontXml : StyleXmlHelper {
		...
		internal ExcelFontXml(XmlNamespaceManager nsm, XmlNode topNode) :
			...
			if (topNode.SelectSingleNode(underLinedPath, NameSpaceManager) != null) {
				...
				_underlineType = ut == "" ? ExcelUnderLineType.Single : (ExcelUnderLineType)Enum.Parse(typeof(ExcelUnderLineType), ut, true);
			} else {
				_underlineType = ExcelUnderLineType.None;
			}
		}
		...
		public static float GetFontHeight(string name, float size) {
			...
			return FontSize.FontHeights.ContainsKey(name) ? GetHeightByName(name, size) : GetHeightByName("Calibri", size);
		}
		...
		private static float GetHeightByName(string name, float size) {
			if (FontSize.FontHeights[name].ContainsKey(size)) {
				...
				if (min == max || max == float.MaxValue) {
					...
					return min == -1
						? Convert.ToSingle(FontSize.FontHeights[name][max].Height)
						: Convert.ToSingle(FontSize.FontHeights[name][min].Height + (FontSize.FontHeights[name][max].Height - FontSize.FontHeights[name][min].Height) * ((size - min) / (max - min)));
				}
			}
		}
