...
namespace OfficeOpenXml.Style.XmlAccess {
	...
	public sealed class ExcelNumberFormatXml : StyleXmlHelper {
		...
		internal ExcelNumberFormatXml(XmlNamespaceManager nameSpaceManager, bool buildIn) : base(nameSpaceManager) => BuildIn = buildIn;
		...
		internal class ExcelFormatTranslator {
			...
			internal CultureInfo Culture {
				get {
					return _ci == null ? CultureInfo.CurrentCulture : _ci;
				}
			...
			private void ToNetFormat(string ExcelFormat, bool forColWidth) {
				...
				for (var pos = 0; pos < ExcelFormat.Length; pos++) {
					var c = ExcelFormat[pos];
					if (c == '"') {
						isText = !isText;
					} else {
						if (ignoreNext) {
							...
							if (c == ']') {
								...
								{
									var li = Regex.Split(bracketText, "-");
									if (li[0].Length > 1) {
			...
			private static void SetDecimal(List<int> lstDec, StringBuilder sb) {
				if (lstDec.Count > 1) {
					for (var i = lstDec.Count - 1; i >= 0; i--) {
			...
			internal string FormatFraction(double d) {
				...
				if (d == 0 || double.IsNaN(d)) {
					return fmt[0].Trim() == "" && fmt[1].Trim() == ""
						? new string(' ', FractionFormat.Length)
						: 0.ToString(fmt[0]) + "/" + 1.ToString(fmt[0]);
				}

				var maxDigits = fmt[1].Length;
				var sign = d < 0 ? "-" : "";
				if (fixedDenominator == 0) {
					...
					var divRes = 1 / ((double)Math.Abs(d) - intPart);
					double result, prevResult = double.NaN;
					int listPos = 2, index = 1;
					while (true) {
						index++;
						var intDivRes = Math.Floor(divRes);
				...
				if (numerator == denomerator || numerator == 0) {
					...
					return intPart == 0
						? sign + FmtInt(numerator, fmt[0]) + "/" + FmtInt(denomerator, fmt[1])
						: sign + intPart.ToString(NetFormat).Replace("?", FmtInt(numerator, fmt[0]) + "/" + FmtInt(denomerator, fmt[1]));
				}
			}
			...
			private string FmtInt(double value, string format) {
				...
				if (v.Length < format.Length) {
					for (var i = format.Length - v.Length - 1; i >= 0; i--) {
						if (format[i] == '?') {
							pad += " ";
						} else if (format[i] == ' ') {
							pad += "0";
