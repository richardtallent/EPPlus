...
namespace OfficeOpenXml.Style.XmlAccess {
	...
	public sealed class ExcelXfs : StyleXmlHelper {
		...
		internal override string Id => XfId + "|" + NumberFormatId.ToString() + "|" + FontId.ToString() + "|" + FillId.ToString() + "|" + BorderId.ToString() + VerticalAlignment.ToString() + "|" + HorizontalAlignment.ToString() + "|" + WrapText.ToString() + "|" + ReadingOrder.ToString() + "|" + isBuildIn.ToString() + TextRotation.ToString() + Locked.ToString() + Hidden.ToString() + ShrinkToFit.ToString() + Indent.ToString() + QuotePrefix.ToString(); //return Numberformat.Id + "|" + Font.Id + "|" + Fill.Id + "|" + Border.Id + VerticalAlignment.ToString() + "|" + HorizontalAlignment.ToString() + "|" + WrapText.ToString() + "|" + ReadingOrder.ToString(); 
		internal ExcelXfs Copy() => Copy(_styles);
		...
		internal int GetNewID(ExcelStyleCollection<ExcelXfs> xfsCol, StyleBase styleObject, eStyleClass styleClass, eStyleProperty styleProperty, object value) {
			...
			return id < 0 ? xfsCol.Add(newXfs.Id, newXfs) : id;
		}
		...
		private int GetIdFill(eStyleClass styleClass, eStyleProperty styleProperty, object value) {
			...
			switch (styleProperty) {
				...
				case eStyleProperty.AutoColor:
					...
					destColor = styleClass == eStyleClass.FillPatternColor ? fill.PatternColor : fill.BackgroundColor;
			...
			return subId == int.MinValue ? _styles.Fills.Add(id, fill) : subId;
		}
		private int GetIdGradientFill(eStyleClass styleClass, eStyleProperty styleProperty, object value) {
			...
			switch (styleProperty) {
				...
				case eStyleProperty.AutoColor:
					...
					destColor = styleClass == eStyleClass.FillGradientColor1 ? fill.GradientColor1 : fill.GradientColor2;
		...
		private int GetIdFont(eStyleProperty styleProperty, object value) {
			...
			return subId == int.MinValue ? _styles.Fonts.Add(id, fnt) : subId;
		}
		internal override XmlNode CreateXmlNode(XmlNode topNode) => CreateXmlNode(topNode, false);
