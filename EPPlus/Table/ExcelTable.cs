...
namespace OfficeOpenXml.Table {
	...
	public class ExcelTable : XmlHelper, IEqualityComparer<ExcelTable> {
		...
		private string GetStartXml(string name, int tblId) {
			...
			var cols = Address._toCol - Address._fromCol + 1;
		...
		internal ExcelAddressBase AutoFilterAddress {
			get {
				var a = GetXmlNodeString(AUTOFILTER_PATH);
				return a == "" ? null : new ExcelAddressBase(a);
			}
		}
		private void WriteAutoFilter(bool showTotal) {
			string autofilterAddress;
			if (ShowHeader) {
				autofilterAddress = showTotal ? ExcelCellBase.GetAddress(Address._fromRow, Address._fromCol, Address._toRow - 1, Address._toCol) : Address.Address;
		...
		public bool ShowTotal {
			...
			set {
				if (value != ShowTotal) {
					Address = value
						? new ExcelAddress(WorkSheet.Name, ExcelAddressBase.GetAddress(Address.Start.Row, Address.Start.Column, Address.End.Row + 1, Address.End.Column))
						: new ExcelAddress(WorkSheet.Name, ExcelAddressBase.GetAddress(Address.Start.Row, Address.Start.Column, Address.End.Row - 1, Address.End.Column));
