...
namespace OfficeOpenXml.Table.PivotTable {
	...
	public class ExcelPivotTableField : XmlHelper {
		...
		public string Name {
			get {
				var v = GetXmlNodeString("@name");
				return v == "" ? _cacheFieldHelper.GetXmlNodeString("@name") : v;
			}
		...
		public bool MultipleItemSelectionAllowed {
			get => GetXmlNodeBool("@multipleItemSelectionAllowed");
			set => SetXmlNodeBool("@multipleItemSelectionAllowed", value);
		}
		...
		public bool ShowDropDowns {
			get => GetXmlNodeBool("@showDropDowns");
			set => SetXmlNodeBool("@showDropDowns", value);
		}
		...
		public bool ShowInFieldList {
			get => GetXmlNodeBool("@showInFieldList");
			set => SetXmlNodeBool("@showInFieldList", value);
		}
		...
		public bool ShowAsCaption {
			get => GetXmlNodeBool("@showPropAsCaption");
			set => SetXmlNodeBool("@showPropAsCaption", value);
		}
		...
		public bool ShowMemberPropertyInCell {
			get => GetXmlNodeBool("@showPropCell");
			set => SetXmlNodeBool("@showPropCell", value);
		}
		...
		public bool ShowMemberPropertyToolTip {
			get => GetXmlNodeBool("@showPropTip");
			set => SetXmlNodeBool("@showPropTip", value);
		}
		#endregion
		...
		public bool IsRowField {
			get => (TopNode.SelectSingleNode(string.Format("../../d:rowFields/d:field[@x={0}]", Index), NameSpaceManager) != null);
			internal set {
				if (value) {
					...
					TopNode.InnerXml = BaseIndex == Index ? "<items count=\"1\"><item t=\"default\" /></items>" : "<items count=\"0\"></items>";
				} else {
					if (TopNode.SelectSingleNode(string.Format("../../d:rowFields/d:field[@x={0}]", Index), NameSpaceManager) is XmlElement node) {
		...
		public bool IsColumnField {
			get => (TopNode.SelectSingleNode(string.Format("../../d:colFields/d:field[@x={0}]", Index), NameSpaceManager) != null);
			internal set {
				if (value) {
					...
					if (TopNode.SelectSingleNode(string.Format("../../d:colFields/d:field[@x={0}]", Index), NameSpaceManager) is XmlElement node) {
						node.ParentNode.RemoveChild(node);
					}
				}
			}
		...
		internal ExcelPivotTableFieldDateGroup SetDateGroup(eDateGroupBy GroupBy, DateTime StartDate, DateTime EndDate, int interval) {
			...
			var items = AddDateGroupItems(group, GroupBy, StartDate, EndDate, interval);
			AddFieldItems(items);
		...
		internal ExcelPivotTableFieldNumericGroup SetNumericGroup(double start, double end, double interval) {
			...
			var items = AddNumericGroupItems(group, start, end, interval);
			AddFieldItems(items);
		...
		private int AddNumericGroupItems(ExcelPivotTableFieldNumericGroup group, double start, double end, double interval) {
			...
			var groupItems = group.TopNode.SelectSingleNode("d:fieldGroup/d:groupItems", group.NameSpaceManager) as XmlElement;
			var items = 2;
			//First date
			var index = start;
			var nextIndex = start + interval;
		...
		private void AddFieldItems(int items) {
			...
			var itemsNode = TopNode.SelectSingleNode("d:items", NameSpaceManager) as XmlElement;
		...
		private int AddDateGroupItems(ExcelPivotTableFieldGroup group, eDateGroupBy GroupBy, DateTime StartDate, DateTime EndDate, int interval) {
			...
			switch (GroupBy) {
				...
				case eDateGroupBy.Days:
					if (interval == 1) {
						var dt = new DateTime(2008, 1, 1); //pick a year with 366 days
						while (dt.Year == 2008) {
		...
		public void AddDateGrouping(eDateGroupBy groupBy) => AddDateGrouping(groupBy, DateTime.MinValue, DateTime.MaxValue, 1);
		...
		public void AddDateGrouping(eDateGroupBy groupBy, DateTime startDate, DateTime endDate) => AddDateGrouping(groupBy, startDate, endDate, 1);
		...
		/// <param name="startDate">Fixed start date. Use DateTime.MinValue for auto</param>
		/// <param name="endDate">Fixed end date. Use DateTime.MaxValue for auto</param>
		public void AddDateGrouping(int days, DateTime startDate, DateTime endDate) => AddDateGrouping(eDateGroupBy.Days, startDate, endDate, days);
		private void AddDateGrouping(eDateGroupBy groupBy, DateTime startDate, DateTime endDate, int groupInterval) {
			...
			var firstField = true;
			var fields = new List<ExcelPivotTableField>();
			//Seconds
		...
		private ExcelPivotTableField AddField(eDateGroupBy groupBy, DateTime startDate, DateTime endDate, ref bool firstField) => AddField(groupBy, startDate, endDate, ref firstField, 1);
