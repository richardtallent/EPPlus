...
namespace OfficeOpenXml.Utils.CompundDocument {
	internal class CompoundDocument {
		...
		internal CompoundDocument(MemoryStream ms) => Read(ms);
		internal CompoundDocument(FileInfo fi) => Read(fi);
		...
		internal CompoundDocument(byte[] doc) => Read(doc);
		internal void Read(FileInfo fi) {
			var b = File.ReadAllBytes(fi.FullName);
			Read(b);
		}
		internal void Read(byte[] doc) => Read(new MemoryStream(doc));
