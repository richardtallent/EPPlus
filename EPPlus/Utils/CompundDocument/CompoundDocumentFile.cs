	...
	internal class CompoundDocumentFile : IDisposable {
		...
		internal void Read(BinaryReader br) {
			...
			foreach (var d in dir) {
				if (d.Stream == null && d.StreamSize > 0) {
					d.Stream = d.StreamSize < _miniStreamCutoffSize
						? GetStream(d.StartingSectorLocation, d.StreamSize, dwi.miniFAT, _miniSectors)
						: GetStream(d.StartingSectorLocation, d.StreamSize, dwi.FAT, _sectors);
				}
			}
		...
		private int WriteDirStream(BinaryWriter bw, List<CompoundDocumentItem> dirs) {
			if (dirs.Count > 0) {
				...
				for (var i = 0; i < Math.Min(_sectorSize / 128, dirs.Count); i++) {
					dirs[i].Write(bw);
				}
			} else {
				return -1;
		...
		private void CheckUpdateDIFAT(BinaryWriter bw) {
			if (bw.BaseStream.Position % _sectorSize == 0) {
				...
				//Add to DIFAT
				var FATSector = (int)(bw.BaseStream.Position / _sectorSize - 1);
		...
		private void AllocateFAT(BinaryWriter bw, int miniFatLength, List<CompoundDocumentItem> dirs) {
			...
			bw.Seek(_currentFATSectorPos, SeekOrigin.Begin);
			var sectorPos = 1;
			for (var i = 1; i < 109; i++)     //We have 1 FAT sector to start with at sector 0
			{
			...
			for (var j = 0; j < _numberofDIFATSectors; j++) {
				...
				for (var i = 0; i < _sectorSizeInt - 1; i++) {
					WriteFATItem(bw, FAT_SECTOR);
					sectorPos++;
		...
		private int GetDIFatSectors(int FATSectors) => FATSectors > 109 ? GetSectors((FATSectors - 109), _sectorSizeInt - 1) : 0;
		...
		private void CreateFATStreams(CompoundDocumentItem item, BinaryWriter bw, BinaryWriter bwMini, DocWriteInfo dwi) {
			if (item.ObjectType != 5)   //Root, we must have the miniStream first.
			{
				if (item.StreamSize > 0) {
					...
					item.StartingSectorLocation = item.StreamSize < _miniStreamCutoffSize
						? WriteStream(bwMini, dwi.miniFAT, item.Stream, miniFATSectorSize)
						: WriteStream(bw, dwi.FAT, item.Stream, FATSectorSizeV3);
				}
			}
			foreach (var c in item.Children) {
