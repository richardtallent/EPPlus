...
namespace OfficeOpenXml.VBA {
	...
	public class ExcelVbaProject {
		...
		private void ReadModules() {
			foreach (var modul in Modules) {
				...
				while (pos + 9 < code.Length && code.Substring(pos, 9) == "Attribute") {
					...
					var lineSplit = linePos > 0 ? code.Substring(pos + 9, linePos - pos - 9).Split('=') : code.Substring(pos + 9).Split(new char[] { '=' }, 1);
						...
						case "DPB":
							...
							if (dpb.Length >= 28) {
								...
								for (var i = 0; i < 24; i++) {
									var bit = 128 >> (int)((i % 8));
									if (i < 4) {
										_protection.PasswordKey[i] = (int)(flags[0] & bit) == 0 ? (byte)0 : keyNoNulls[i];
									} else {
										var flagIndex = (i - i % 8) / 8;
										_protection.PasswordHash[i - 4] = (int)(flags[flagIndex] & bit) == 0 ? (byte)0 : hashNoNulls[i - 4];
									}
								}
							}
							break;
		...
		private string WriteVisibilityState() => Encrypt(new byte[] { (byte)(Protection.VisibilityState ? 0xFF : 0) });
		#endregion
