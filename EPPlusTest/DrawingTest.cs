...
namespace EPPlusTest {
	...
	public class DrawingTest : TestBase {
		...
		public void ReadDrawing() {
			using (var pck = new ExcelPackage(new FileInfo(_worksheetPath + @"Drawing.xlsx"))) {
		...
		public void Scatter() {
			...
			chrt.VaryColors = true;
			var ser = chrt.Series[0] as ExcelScatterChartSerie;
		...
		public void ReadMultiChartSeries() {
			var pck = new ExcelPackage(new FileInfo("c:\\temp\\chartseries.xlsx"), true);

			var ws = pck.Workbook.Worksheets[1];
			var c = ws.Drawings[0] as ExcelChart;
		...
		public void AllDrawingsInsideMarkupCompatibility() {
...
#else
			var workbooksDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\workbooks");
#endif
			...
			var path = Path.Combine(workbooksDir, "AllDrawingsInsideMarkupCompatibility.xlsm");

			// Load example document.
			_pck = new ExcelPackage(new FileInfo(path));
			// Verify the drawing part exists:
			var partUri = new Uri("/xl/drawings/drawing1.xml", UriKind.Relative);
			...
			var drawingsXml = new XmlDocument();
			...
			// Save it out.
			var savedPath = Path.Combine(workbooksDir, "AllDrawingsInsideMarkupCompatibility2.xlsm");
