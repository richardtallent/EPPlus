...
namespace EPPlusTest {
	...
	public class Encrypt : TestBase {
		...
		public void ReadWriteEncrypt() {
			using (var pck = new ExcelPackage(new FileInfo(@"Test\Drawing.xlsx"), true)) {
			...
			using (var pck = new ExcelPackage(new FileInfo(_worksheetPath + @"\DrawingEncr.xlsx"), true, "EPPlus")) {
				pck.Encryption.IsEncrypted = false;
				pck.SaveAs(new FileInfo(_worksheetPath + @"\DrawingNotEncr.xlsx"));
			}

			var fs = new FileStream(_worksheetPath + @"\DrawingEncr.xlsx", FileMode.Open, FileAccess.ReadWrite);
			using (var pck = new ExcelPackage(fs, "EPPlus")) {
				pck.Encryption.IsEncrypted = false;
				pck.SaveAs(new FileInfo(_worksheetPath + @"DrawingNotEncr.xlsx"));
			}
		...
		public void WriteProtect() {
			var package = new ExcelPackage(new FileInfo(@"c:\temp\workbookprot2.xlsx"), "");
			//Load the sheet with one string column, one date column and a few random numbers.
