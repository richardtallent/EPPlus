...
namespace EPPlusTest {
	...
	public class Issues : TestBase {
		...
		public void Issue15058() {
			var newFile = new System.IO.FileInfo(@"C:\Temp\output.xlsx");
			var excelP = new ExcelPackage(newFile);
			ExcelWorksheet ws = excelP.Workbook.Worksheets[1];
		}
		[Ignore]
		[TestMethod]
		public void Issue15063() {
			var newFile = new System.IO.FileInfo(@"C:\Temp\bug\TableFormula.xlsx");
			var excelP = new ExcelPackage(newFile);
			ExcelWorksheet ws = excelP.Workbook.Worksheets[1];
			ws.Calculate();
		}
		...
		public void Issue15112() {
			var case1 = new System.IO.FileInfo(@"c:\temp\bug\src\src\DeleteRowIssue\Template.xlsx");
		...
		public void Issue15109() {
			var newFile = new System.IO.FileInfo(@"C:\Temp\bug\test01.xlsx");
			var excelP = new ExcelPackage(newFile);
			ExcelWorksheet ws = excelP.Workbook.Worksheets[1];
		...
		public void Issue15145() {
			using (var p = new ExcelPackage(new System.IO.FileInfo(@"C:\Temp\bug\ColumnInsert.xlsx"))) {
		...
		public void Issue15169() {
			var fileInfo = new FileInfo(@"C:\temp\bug\issue\input.xlsx");

			var excelPackage = new ExcelPackage(fileInfo);
			{
				var sheetName = "Labour Costs";
		...
		public void Issue15172() {
			var fileInfo = new FileInfo(@"C:\temp\bug\book2.xlsx");
		...
		public void Issue15174() {
			using (var package = new ExcelPackage(new FileInfo(@"C:\temp\bug\MyTemplate.xlsx"))) {
		...
		[Ignore]
		[TestMethod]
		public void Issue15154() => Directory.EnumerateFiles(@"c:\temp\bug\ConstructorInvokationNotThreadSafe\").AsParallel().ForAll(file => {
			//lock (_lock)
			//{
		...
		public void Issue15188() {
			using (var package = new ExcelPackage()) {
				...
				var a = worksheet.Cells[2, 6].Text;
		...
		public void Issue15167() {
			var fileInfo = new FileInfo(@"c:\temp\bug\Draw\input.xlsx");

			var excelPackage = new ExcelPackage(fileInfo);
			{
				var sheetName = "Board pack";
		...
		/**** Pivottable issue ****/
		public void Issue() {
			var outputDir = new DirectoryInfo(@"c:\ExcelPivotTest");
			var MyFile = new FileInfo(@"c:\temp\bug\pivottable.xlsx");
			LoadData(MyFile);
		...
		private void LoadData(FileInfo MyFile) {
			...
			using (var EP = new ExcelPackage(MyFile)) {
		...
		public void Issues14699() {
			var newFile = new FileInfo(string.Format("c:\\temp\\bug\\EPPlus_Issue14699.xlsx", System.IO.Directory.GetCurrentDirectory()));
			var pkg = new ExcelPackage(newFile);
		...
		public void Issue15429() {
			var file = new FileInfo(@"c:\temp\original.xlsx");
		...
		public void Issue15436() {
			var file = new FileInfo(@"c:\temp\incorrect value.xlsx");
			using (var excelPackage = new ExcelPackage(file)) {
		...
		public void Issue13128() {
			var file = new FileInfo(@"c:\temp\students.xlsx");
			using (var excelPackage = new ExcelPackage(file)) {
		...
		public void Issue15469() {
			var excelPackage = new ExcelPackage(new FileInfo(@"c:\temp\bug\EPPlus-Bug.xlsx"), true);
			using (var fs = new FileStream(@"c:\temp\bug\EPPlus-Bug-new.xlsx", FileMode.Create)) {
		...
		public void MergeIssue() {
			...
			var fi = new FileInfo(Path.Combine(worksheetPath, "Example.xlsx"));
			fi.Delete();
			using (var pckg = new ExcelPackage(fi)) {
		...
		public void Issue15566() {
			var TemplateFileName = @"c:\temp\bug\TestWithPivotTablePointingToExcelTableForData.xlsx";
			var ExportFileName = @"c:\temp\bug\TestWithPivotTablePointingToExcelTableForData_Export.xlsx";
			using (var excelpackage = new ExcelPackage(new FileInfo(TemplateFileName), true)) {
		...
		public void BugCommentNullAfterRemove() {
			...
			var xls = @"c:\temp\bug\in.xlsx";

			var theExcel = new ExcelPackage(new FileInfo(xls), true);

			ExcelRangeBase cell;
			ExcelComment cmnt;
			ExcelWorksheet ws = theExcel.Workbook.Worksheets[1];

			foreach (var addr in "B4 B11".Split(' ')) {
				cell = ws.Cells[addr];
				cmnt = cell.Comment;
		...
		public void BugCommentExceptionOnRemove() {
			...
			foreach (var addr in "B4 B16".Split(' ')) {
				cell = ws.Cells[addr];
				cmnt = cell.Comment;
		...
		public void Issuer27() {
			var file = new FileInfo(@"C:\Temp\Test.xlsx");
		...
		public void Issuer26() {
			var file = new FileInfo(@"C:\Temp\repeatrowcol.xlsx");
			var pck = new ExcelPackage(file);
			ExcelWorksheet ws = pck.Workbook.Worksheets.Add("Worksheet");
		...
		public void Issue32() {
			...
			var newFileName = outputDir.FullName + @"\sample1_copied.xlsx";
		...
		[TestMethod]
		public void Issue61() {
			var table1 = new DataTable("TestTable");
		...
		[TestMethod, Ignore]
		public void Issue55() {
			var pck = new ExcelPackage(new FileInfo(@"C:\Temp\bug\monocell.xlsx"));
		...
		public void Issue51() {
			...
			using (var pck = new ExcelPackage(filename)) {
		...
		public void Issue100() {
			...
			var outputFile = new FileStream(@"c:\temp\bug\epplus_drawing_id_issue_new.xlsx", FileMode.Create, FileAccess.ReadWrite, FileShare.None);
			using (var package = new ExcelPackage(templateFile)) {
				ExcelWorkbook wb = package.Workbook;
				ExcelWorksheet sh = wb.Worksheets[1];
				var img_ = System.Drawing.Image.FromFile(@"C:\temp\img\background.gif");
		...
		public void Issue99() {
			...
			using (var inStream = File.Open(template, FileMode.Open)) {
				using (var outStream = File.Open(result, FileMode.OpenOrCreate, FileAccess.ReadWrite)) {
					using (var xl = new ExcelPackage(outStream, inStream)) {
						xl.Save();
					}
				}
			}
		...
		public void Issue107() {
			using (var epIN = new ExcelPackage(new FileInfo(@"C:\temp\bug\issue107\in.xlsx")))
			using (var epOUT = new ExcelPackage(new FileInfo(@"C:\temp\bug\pivotbug107.xlsx"))) {
		...
		public void Issue155() {
			using (var pck = new ExcelPackage()) {
				...
				var path = @"C:\temp\test.xlsx";
				File.WriteAllBytes(path, data);
			}
		}
		[TestMethod, Ignore]
		public void Issue173() {
			using (var xlPackage = new ExcelPackage(new FileInfo($@"C:\temp\bug\issue173.xlsx"))) {
		...
		public class Car {
			...
			internal static List<Car> GenerateList() => new List<Car>
			{
				//random data
		...
		public void Issue348() {
			using (var pck = new ExcelPackage()) {
				...
				var formula = "VLOOKUP(C2,A:B,1,0)";
