...
namespace EPPlusTest {
	...
	public class VBATests {
		...
		public void Compression() {
			...
			var value = "#aaabcdefaaaaghijaaaaaklaaamnopqaaaaaaaaaaaarstuvwxyzaaa";

			var compValue = VBACompression.CompressPart(Encoding.GetEncoding(1252).GetBytes(value));
			var decompValue = Encoding.GetEncoding(1252).GetString(VBACompression.DecompressPart(compValue));
		...
		public void ReadVBA() {
			...
			var ret = new List<X509Certificate2>();
		...
		public void WriteLongVBAModule() {
			...
			var code = new StringBuilder("Public Sub CreateData()\r\n");
		...
		public void VbaError() {
			var workingDir = new DirectoryInfo(@"C:\epplusExample\folder");
			if (!workingDir.Exists) workingDir.Create();
			var f = new FileInfo(workingDir.FullName + "//" + "temp.xlsx");
			if (f.Exists) f.Delete();
			var myPackage = new ExcelPackage(f);
			...
			var f2 = new FileInfo(workingDir.FullName + "//" + "newfile.xlsm");
			ExcelVBAModule excelVbaModule = myPackage.Workbook.VbaProject.Modules.AddModule("Module1");
			var mybuilder = new StringBuilder(); mybuilder.AppendLine("Sub Jiminy()");
