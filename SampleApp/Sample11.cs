...
namespace EPPlusSamples {
	...
	class Sample11 {
		...
		private static void PrintTimeValidationDetails(ExcelWorksheet sheet, ExcelDataValidationTime validation, int row) {
			...
			value1 = !string.IsNullOrEmpty(validation.Formula.ExcelFormula)
				? validation.Formula.ExcelFormula
				: string.Format("{0}:{1}:{2}", validation.Formula.Value.Hour, validation.Formula.Value.Minute, validation.Formula.Value.Second ?? 0);
