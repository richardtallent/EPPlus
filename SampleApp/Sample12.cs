...
namespace EPPlusSamples {
	...
	public static class Sample12 {
		...
		public static string RunSample12(string connectionStr) {
			...
			list = connectionStr == "" ? GetRandomData() : GetDataFromSQL(connectionStr);
		...
		private static List<SalesDTO> GetDataFromSQL(string connectionStr) {
			...
			using (var sqlConn = new SqlConnection(connectionStr)) {
				sqlConn.Open();
				using (var sqlCmd = new SqlCommand("select Title, FirstName, MiddleName, LastName, SubTotal, OrderDate, TaxAmt, Freight, TotalDue  from Sales.SalesOrderHeader s inner join HumanResources.Employee h on s.SalesPersonID = h.BusinessEntityID inner join Person.Person c on c.BusinessEntityID = h.BusinessEntityID order by LastName, FirstName, MiddleName;", sqlConn)) {
